...
if errorlevel 1 goto NOJRE18_64
...
if errorlevel 1 goto NOJRE18
for /f "tokens=2* delims=	 " %%A in (%TMP1%) do set JAVA_HOME=%%B
if not errorlevel 1 goto GOTJAVA

:NOJRE18
reg query "HKEY_LOCAL_MACHINE\SOFTWARE\JavaSoft\Java Development Kit\1.8" /v JavaHome >%TMP1% 2>nul
if errorlevel 1 goto NOJDK18_64
::  note: delims is a TAB followed by a space
for /f "tokens=2* delims=	 " %%A in (%TMP1%) do set JAVA_HOME=%%B
if not errorlevel 1 goto GOTJAVA

:NOJDK18_64
reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\JavaSoft\Java Development Kit\1.8" /v JavaHome >%TMP1% 2>nul
if errorlevel 1 goto NOJAVA
for /f "tokens=2* delims=	 " %%A in (%TMP1%) do set JAVA_HOME=%%B
if not errorlevel 1 goto GOTJAVA

:NOJAVA
echo Could not find Java 1.8 in registry - please ensure it is installed correctly.
goto END
