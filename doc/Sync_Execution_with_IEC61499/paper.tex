...
be used in suprevisory control theory. In the theory these
...
\label{sec:sct}
Suprevisory control theory provides algorithms for
calcuation of allowed behavior of the system from discrete
...
and the result of the computation is called the suprevisor.
...
transition occur. Special case is transition labled with
...
Event demultiplexer does the opossite and generates an event
...
implementaion of supervisory control theory are presented in
...
the application is manualy crafted using informal
specificatons of the system behavior, and transfered to the
PLC-code. For each application the communication between the
suprevisor and the plants has to be implemented in a
...
plants are upgraded or chaged the procedure of supervisor
...
so that a supervisor can be synthesised from automata models
...
correct modeling of the plants and transfering of the
...
Fig.~\ref{fig:app} for the example. The sekelton in this
...
until now it was dificult to apply the theory to
