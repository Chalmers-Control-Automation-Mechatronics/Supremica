...
//###########################################################################
//# $Id: ControlledSurface.java,v 1.1 2005-02-17 01:43:35 knut Exp $
//###########################################################################
...
public class ControlledSurface extends EditorSurface implements MouseMotionListener, MouseListener, KeyListener {
    ...
    public void mouseDragged(MouseEvent e) {
	    ...
	    for(int i = 0; i<nodes.size(); i++) {
        	if(((EditorNode)nodes.get(i)).getPosition().distance(dx, dy) < selectedNode.getWidth()
		   && ((EditorNode)nodes.get(i)) != selectedNode) {
		    return;
	        }
	    }

	    for(int i = 0; i<nodeGroups.size(); i++) {
		if(((EditorNodeGroup)nodeGroups.get(i)).getBounds().intersects(new Rectangle(dx - EditorNode.WIDTH/2,
											     dy - EditorNode.WIDTH/2,
											     EditorNode.WIDTH, EditorNode.WIDTH))  && 
		   !((EditorNodeGroup)nodeGroups.get(i)).getBounds().contains(new Rectangle(dx - EditorNode.WIDTH/2,
											    dy - EditorNode.WIDTH/2,
											    EditorNode.WIDTH, EditorNode.WIDTH))){
		    return;
		}
	...
	if(selectedNodeGroup != null && (T.getPlace() == "select" || T.getPlace() == "nodegroup")){
		...
		if (nodesSnap){
		    selectedNodeGroup.moveGroup(Math.round((e.getX()-xoff)/gridSize)*gridSize,
						Math.round((e.getY()-yoff)/gridSize)*gridSize);
		}
		else{		
	...
	if(selectedLabelGroup != null && T.getPlace() == "select"){
        ...
        if(selectedNode != null && T.getPlace() == "edge") {
            int[] dat = {selectedNode.getX(), selectedNode.getY(), e.getX(), e.getY()};
            if(lines != null && lines.size() > 0) {
                lines.set(0, dat);
            }
            else {
    ...
    public void mouseClicked(MouseEvent e) {
		...
		dy = e.getY();
            ...
            if(getObjectAtPosition(dx, dy) == null ||
               getObjectAtPosition(dx, dy).getType() != EditorObject.NODE) {
