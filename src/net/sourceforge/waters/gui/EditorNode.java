...
//# CLASS:   EditorNode
//###########################################################################
//# $Id: EditorNode.java,v 1.1 2005-02-17 01:43:35 knut Exp $
//###########################################################################
 ...
 * @author Gian Perrone
 */
public class EditorNode extends EditorObject
{
	...
	return (((position.getX() - WIDTH/2) <= Cxposition) && (Cxposition <= (position.getX() + WIDTH/2))
		&& ((position.getY() - WIDTH/2) <= Cyposition) && (Cyposition <= (position.getY() + WIDTH/2)));
    }
	...
	Arc2D.Double a = new Arc2D.Double();
		...
		g2d.drawLine((int)position.getX(), (int)position.getY()-(WIDTH/2),
			     (int)position.getX(), (int)position.getY()+(WIDTH/2));
		g2d.drawLine((int)position.getX()-(WIDTH/2), (int)position.getY(),
			     (int)position.getX()+(WIDTH/2), (int)position.getY());
	    }
        }
	else {
	    if (!i.hasNext()){
