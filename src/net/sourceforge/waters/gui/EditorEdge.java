...
//# CLASS:   EditorEdge
//###########################################################################
//# $Id: EditorEdge.java,v 1.1 2005-02-17 01:43:35 knut Exp $
//###########################################################################
 ...
 * @author Simon Ware
 */
public class EditorEdge extends EditorObject
{
    private int angle;
	...
	if (proxy.getGeometry() == null){    
	    ...
	    tPoint = new Point2D.Double(((Point2D)proxy.getGeometry().getPoints().get(0)).getX(),
					((Point2D)proxy.getGeometry().getPoints().get(0)).getY());
	...
	if (startNode == endNode)
	{
	    tPoint.setLocation(tPoint.getX() + (endNode.getX() - (int)ox)/2,
			       tPoint.getY() + (endNode.getY() - (int)oy)/2);
	    return;
	}
    ...
    private boolean onLine(double x, double y) {
    	Rectangle2D.Double r = new Rectangle2D.Double((double)start.getX(), (double)start.getY(),
						      (double)(endNode.getX() - start.getX()),(double)(endNode.getY() - start.getY())); 
	if (r.contains(x,y)){
    ...
    {
	double dist = (double)Math.sqrt((getCPointX()-start.getX()) * (getCPointX()-start.getX()) +
					(getCPointY()-start.getY()) * (getCPointY()-start.getY()))*tearRatio;           
	double r = dist/2;
	...
	//return (Math.abs(Cxposition-controlPoint.getX()) < 5 && Math.abs(Cyposition-controlPoint.getY()) < 5);
        ...
        int n = 0;
        Line2D.Double l = new Line2D.Double(start.getX(), start.getY(), endNode.getX(), endNode.getY());
        q = new QuadCurve2D.Double(start.getX(),
                                   start.getY(),
                                   getCPointX(),
                                   getCPointY(),
                                   endNode.getX(),
    ...
    public void drawObject(Graphics g)
    {
	    ...
	    drawArrow(getCPointX(), getCPointY(), start.getX(), start.getY(), (int)getTPointX(), (int)getTPointY(), true, g2d);
            ...
            g2d.draw(new QuadCurve2D.Double(start.getX(),
                                            start.getY(),
                                            getCPointX(),
                                            getCPointY(),
                                            endNode.getX(),
