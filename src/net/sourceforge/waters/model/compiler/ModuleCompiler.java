  ...
  extends AbstractModuleProxyVisitor
{
    ...
    throws VisitorException
  {
    ...
    if (list.isEmpty()) {
      final EmptyLabelBlockException exception =
        new EmptyLabelBlockException(proxy);
      throw wrap(exception);
    }
    ...
    throws VisitorException
  {
    if (source instanceof SimpleNodeProxy) {
      ...
      createTransitions
        (simpleSource, events, target, groupEntry, deterministic);
    } else {
      for (final NodeProxy child : source.getImmediateChildNodes()) {
        createTransitions(child, events, target, groupEntry, deterministic);
      }
    }
  }
    ...
    throws VisitorException
  {
    if (target instanceof SimpleNodeProxy) {
      ...
      createTransitions
        (source, events, simpleTarget, groupEntry, deterministic);
    } else {
      for (final NodeProxy child : target.getImmediateChildNodes()) {
        createTransitions(source, events, child, groupEntry, deterministic);
      }
    }
  }
