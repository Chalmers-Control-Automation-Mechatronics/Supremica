...
public abstract class JAXBAbstractModuleImporter
  <D extends DocumentProxy, T extends NamedType>
  extends JAXBDocumentImporter<D,T>
{
  //#########################################################################
  //# Constructors
  public JAXBAbstractModuleImporter(final ModuleProxyFactory factory,
                                    final OperatorTable optable)
  {
    ...
    handler = new ImportHandler() {
      @Override
      public FunctionCallExpressionProxy importElement(final ElementType element)
        throws WatersUnmarshalException
      {
        final FunctionCallExpression downcast = (FunctionCallExpression) element;
        return importFunctionCallExpression(downcast);
      }
    };
    mHandlerMap.put
      (net.sourceforge.waters.xsd.module.FunctionCallExpression.class, handler);
    handler = new ImportHandler() {
      @Override
    ...
    handler = new ImportHandler() {
      @Override
      public ModuleSequenceProxy importElement(final ElementType element)
        throws WatersUnmarshalException
      {
        final ModuleSequence downcast = (ModuleSequence) element;
        return importModuleSequence(downcast);
      }
    };
    mHandlerMap.put
      (net.sourceforge.waters.xsd.module.ModuleSequence.class, handler);
    handler = new ImportHandler() {
      @Override
  ...
  //#########################################################################
  //# Importing Documents
  ModuleProxy importModule(final Module element, final URI uri)
      throws WatersUnmarshalException
  {
    ...
    final List<EventDeclProxy> eventDeclList = new LinkedList<EventDeclProxy>();
    ...
    return mFactory.createModuleProxy(name, comment, uri, constantAliasList,
                                      eventDeclList, eventAliasList, componentList);
  }

  ModuleSequenceProxy importModuleSequence(final ModuleSequence element,
                                           final URI uri)
    throws WatersUnmarshalException
  {
    final String name = element.getName();
    final String comment = element.getComment();
    final List<ModuleProxy> list = new LinkedList<ModuleProxy>();
    mModuleSequenceListHandler.fromJAXB(this, element, list);
    return mFactory.createModuleSequenceProxy(name, comment, uri, list);
  }
...
//EFA----------------------
  ...
  private FunctionCallExpressionProxy importFunctionCallExpression
    (final FunctionCallExpression element)
    throws WatersUnmarshalException
  {
    final String text = element.getText();
    final String name = element.getFunctionName();
    final List<SimpleExpressionProxy> args =
      new LinkedList<SimpleExpressionProxy>();
    final List<SimpleExpressionType> argsElement = element.getArguments();
    for (final SimpleExpressionType argElement : argsElement) {
      final SimpleExpressionProxy argProxy =
        (SimpleExpressionProxy) importElement(argElement);
      args.add(argProxy);
    }
    return mFactory.createFunctionCallExpressionProxy(text, name, args);
  }
  ...
  private ModuleSequenceProxy importModuleSequence
      (final ModuleSequence element)
    throws WatersUnmarshalException
  {
    return importModuleSequence(element, null);
  }
