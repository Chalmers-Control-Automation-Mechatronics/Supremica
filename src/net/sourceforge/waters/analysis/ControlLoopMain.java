...
/**
 * <P>A sample main class for testing the {@link ControlLoopChecker}
 * class.</P>
 *
 ...
 * passes it to a control loop checker, and prints the result.
 * More precisely, this class can be run as follows.</P>
 *
 * <P><CODE>java ControlLoopMain
...
public class ControlLoopMain
{
   ...
   * Please refer to the class documentation ({@link ControlLoopMain})
    ...
    try {
      ...
      for (int i = 0; i < args.length; i++) {
        ...
        final ControlLoopChecker checker =
          new ControlLoopChecker(des, desFactory);
        System.out.print(des.getName() + " ... ");
        System.out.flush();

        final boolean result = checker.run();
        if (result) {
          System.out.println("Control loop free");
        } else {
          System.out.println("Contains control loop");
          System.out.println("Counterexample:");
          final LoopTraceProxy counterex = checker.getCounterExample();
