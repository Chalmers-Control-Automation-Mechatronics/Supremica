...
public class TraceFinder
{
  ...
  //#########################################################################
  //# Constructors
  public TraceFinder(final AutomatonProxy aut)
    throws OverflowException
  {
    final Collection<EventProxy> empty = Collections.emptyList();
    mEventEncoding =
      new EventEncoding(aut, empty, EventEncoding.FILTER_PROPOSITIONS);
   ...
   *         accepted.
   */
  public int accepts(final TraceProxy trace)
  {
    if (mInitialStates.isEmpty()) {
      ...
      if (mStateEncoding == null) {
        ...
        while (current != null) {
          ...
          final EventProxy event = events.get(depth - 1);
          final int eventID = mEventEncoding.getEventCode(event);
          iter.reset(currentStateID, eventID);
          if (iter.advance() && iter.advance()) {
            mPath[depth] = mStateEncoding.getState(nextStateID);
          }
   ...
   * {@link #accepts(TraceProxy) accepts()}
