...
public class PrepareForComposition
{
  ...
  public PrepareForComposition(TransitionRelation transitionrelation,
                               EventProxy marked, Set<EventProxy> selfloops)
  {
  ...
  public AutomatonProxy run(ProductDESProxyFactory factory)
  {
    ...
    for (int s = 0; s < mTransitionRelation.numberOfStates(); s++) {
      ...
      Set<EventProxy> used = new THashSet<EventProxy>();
    ...
    for (TIntHashSet ann : statesWithAnnotation.keySet()) {
      ...
      while (!states.isEmpty()) {
        ...
        Collection<EventProxy> used = new THashSet<EventProxy>();
        while (it.hasNext()) {
          int event = it.next();
    ...
    for (int i = 0; i < newStates.size(); i++) {
      ...
      for (int j = 0; j < taus.size(); j++) {
        ...
        for (EventProxy event : mSelfloops) {
          newTransitions.add(factory.createTransitionProxy(source, event, target));
        }
      }
    }
    for (StateProxy s : nextStates) {
      for (EventProxy e : mSelfloops) {
