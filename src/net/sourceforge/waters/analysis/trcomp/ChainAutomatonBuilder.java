...
public class ChainAutomatonBuilder extends AbstractAutomatonBuilder
  implements AutomatonBuilder
{

  //#########################################################################
  //# Constructor
  public ChainAutomatonBuilder(final ProductDESProxyFactory factory,
                            final ChainTRSimplifier simp) {
    super(factory);
    mSimp = simp;
  }
  ...
  public List<Option<?>> getOptions(final OptionMap db)
  {
    ...
    for (int s=0; s<mSimp.size(); s++) {
      options.addAll((mSimp.getStep(s).getOptions(db)));
    }
  ...
  public void setOption(final Option<?> option)
  {
    ...
    for (int s=0; s<mSimp.size(); s++) {
      mSimp.getStep(s).setOption(option);
    }
  }
  ...
  private final ChainTRSimplifier mSimp;
