...
public class MonolithicConflictChecker extends AbstractConflictChecker {
	 ...
	 *            conflict checker considers only those states as marked that
	...
	public boolean run() throws OverflowException {
		try {
			...
			/*
			 * boolean ok = true; while (it.hasNext()) { final int stateid =
			 * it.next(); ok &= exploreBackwards(stateid, coreachable,
			 * MAXDEPTH); } while (!ok) { ok = true; for (int stateid = 0;
			 * stateid < numstates; stateid++) { if (coreachable.get(stateid)) {
			...
			final SyncStateSchema stateSchema = new SyncStateSchema(model,
					mEventMap, marking, preconditionMarking);
			boolean ok = true;
			...
			// is nonblocking
			// final int[] premarked =
			// mSyncProduct.mPreconditionMarkedStates.toArray();
			/*
			 * final TIntIterator iter = premarked.iterator(); boolean
			 * nonblocking = true; while (iter.hasNext()) { final int stateid =
			 * iter.next(); if(!coreachable.get(stateid)) { nonblocking = false;
			 * break; } }
			 */
			boolean nonblocking = true;
			if (preconditionMarking != null) {
				for (int stateid = 0; stateid < 0; stateid++) {
					final long state = mSyncProduct.getStateFromId(stateid);
					// Decode the state.
					final int[] dstate = stateSchema.decodeState(state);
					// final int[] rstate = new int[dstate.length];
					final AutomatonSchema[] automata = stateSchema
							.getOrdering();
					assert automata.length == dstate.length;
			...
			if (nonblocking) {
				...
				// transition to a state with the lowest id wil give a
				// counterexample.
				...
				/*
				 * }else { for (int v = 0; v < numstates; v++) { if
				 * (premarked.contains(v)) { if (!coreachable.get(v)) {
				 * trace_start = v; break; } } } }
				 */
				...
				while (trace_start != 0) {
					...
					countertrace.add(0, mEventMap.getEvent(trans.store
							.getEvent(lowtrans)));
				...
				final ConflictTraceProxy trace = desFactory
						.createConflictTraceProxy(tracename, model,
								countertrace, ConflictKind.CONFLICT);
	...
	private boolean exploreBackwards(final int stateid,
			final BitSet coreachable, int maxdepth) {
		if (maxdepth-- > 0) {
				...
				if (incoming != null) {
					...
					while (it.hasNext()) {
						...
						if (!coreachable.get(src)) {
							result &= exploreBackwards(src, coreachable,
									maxdepth);
						}
					}
				}
	...
	private class SyncProduct {
		...
		// # Auxiliary Methods
		/**
		 * Register a state in the synchronous product. If the state already
		 * exists in the state mapping it will be given the existing id,
		 * otherwise a new mapping will be assigned and the id returned.
		 */
		...
		/**
		 * Look up the state id the state list to get its 64 bit encoded
		 * version.
		 */
		...
		/**
		 * Expands the first available state in the fringe. It is invalid to
		 * call this method with an empty fringe.
		 */
		private void expand() throws OverflowException {
			 ...
			 * boolean need_premarking = true; for (int i = 0; i <
			 * automata.length; i++) { if
			...
			nextevent: for (int eventid = 0; eventid < mEventMap.size(); eventid++) {
				...
				if (stateid != newid) {
					transitions.addTransition(transitions.transitionFactory(
							stateid, newid, eventid));
				}
			}
		}
		...
		private final TransitionSchema transitions = new TransitionSchema(
				false, true);
		...
		/**
		 * A map of synchronous product states to integer ids, to allow the
		 * states to be more efficiently encoded.
		 */
	...
	/**
	 * Represents a 'schema' for interpreting a state encoding for the
	 * synchronous composition of automata.
	 * 
	 * Automata will be assigned an ordering when the schema is created, and
	 * this is assumed when encoding states.
	 */
	private static class SyncStateSchema {
		...
		// # Constructor
		/**
		 * Construct a state schema given the automata to be used in the
		 * synchronous product.
		 */
		private SyncStateSchema(final ProductDESProxy model,
			...
			final List<AutomatonSchema> list = new ArrayList<AutomatonSchema>(
					numaut);
			for (final AutomatonProxy aut : automata) {
				switch (aut.getKind()) {
				case PLANT:
				case SPEC:
					final AutomatonSchema schema = new AutomatonSchema(aut,
							eventmap, marking, preconditionMarking);
					list.add(schema);
					break;
				default:
					break;
		...
		/**
		 * A convenience method that encodes the state from an array of state
		 * proxy objects by looking up the state indexes in the automata schema.
		 */
		...
		/**
		 * Encode a synchronous product state from an array of state indexes.
		 * The ordering of the states should match the ordering of automata
		 * given by the getOrdering method.
		 */
		...
		/**
		 * Decode a state from the synchronous product into its states in
	...
	private static class AutomatonSchema {
		...
		/**
		 * Returns the successor state for the given source state and event, or
		 * -1 if the event is not enabled.
		 */
		 ...
		 * state array.
		 */
		private final TObjectIntHashMap<StateProxy> mStateMap = new TObjectIntHashMap<StateProxy>();
	...
	private static class TransitionSchema {
		...
		// # Auxiliary Methods
		/**
		 * Get, or make transition data. This will get an existing transition
		 * data object for the given state, or it will create a new one and add
		 * it.
		 */
	...
	/**
	 * An attempt to efficiently allocate memory for transitions in an
	 * automaton. This works by allocating into arrays. There is very little
	 * overhead as all transitions have the same lifetime and so deallocating is
	 * not an issue.
	 * 
	...
	private static class TransitionStore {
		...
		// # Simple Access
		/**
		 * Allocate a transition and set its source, destination and event
		 * values.
		 */
	...
	/**
	 * Data for encoding an automaton's state in the synchronous product. Bits
	 * is the number of bits needed to represent the state, and shift is the
	 * amount required to shift the value into position.
	 * 
	 * |--------------------0011--------| |4 | bits |-8------| shift
	 */
	private static class StateEncodingData {
		...
		int decode(long sstate) {
			...
			assert val < Integer.MAX_VALUE : "Decoded value is way too big! ("
					+ val + ")";
			return (int) val;
		}
