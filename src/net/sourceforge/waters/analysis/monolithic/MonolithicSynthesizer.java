...
public boolean run() throws AnalysisException
{
  try {
    ...
    while (!mUnvisited.isEmpty()) {
      ...
      mInitialReachabilityExplorer.explore(tuple);
    }
      ...
      for (int e = 0; e < mNumUncontrollableEvents; e++) {
        states: for (int state = 0; state < mStateTuples.size(); state++) {
          if (!mBadStates.get(state)) {
            ...
            for (int a = 0; a < eventAutomata[e].length; a++) {
              final int aut = eventAutomata[e][a];
              final int source = mStateTuples.get(state)[aut];
              if (transitions[aut][e][source] == null) {
                if (aut < mNumPlants) {
                  continue states;
                } else {
...
protected void setUp() throws AnalysisException
{
  ...
  /*final int[][][][]*/ transitions = new int[mNumAutomata][mNumProperEvents][][];
  /*final int[][][][]*/ reverseTransitions = new int[mNumAutomata][mNumProperEvents][][];
  /*final int[][]*/ eventAutomata = new int[mNumProperEvents][];
  /*final int[][]*/ reverseEventAutomata = new int[mNumProperEvents][];
  ...
  mNumStates = 0;
  mUnvisited = new ArrayDeque<int[]>(100);
  mInitialReachabilityExplorer = new InitialReachabilityExplorer(eventAutomata, transitions, mNDTuple, 0, mNumProperEvents-1);
  mInitialReachabilityExplorer.permutations(mNumAutomata, null, -1, -1);
  mNDTuple2 = Arrays.copyOf(mNDTuple, mNumAutomata);
  mFinalReachabilityExplorer = new FinalReachabilityExplorer(eventAutomata, transitions, mNDTuple2, 0, mNumProperEvents-1);
  mCoreachabilityExplorer = new CoreachabilityExplorer(reverseEventAutomata, reverseTransitions, mNDTupleRvs, 0, mNumProperEvents-1);
  mSuccessorStatesExplorer = new SuccessorStatesExplorer(eventAutomata, transitions, mNDTuple, 0, mNumUncontrollableEvents-1);
}
...
private class UncontrollableAutomatonEventInfo extends
  AutomatonEventInfo
{
...
private class ControllableAutomatonEventInfo extends
  AutomatonEventInfo
{
...
{
  public StateExplorer(final int[][] eventAutomata, final int[][][][] transitions, final int[][] NDTuple, final int firstEvent, final int lastEvent)
  {
    mEventAutomata = eventAutomata;
    mTransitions = transitions;
    mNDTuple = NDTuple;
    mFirstEvent = firstEvent;
    mLastEvent = lastEvent;
  }

  public void explore(final int[] stateTuple) throws OverflowException
  {
    final int state = mStates.get(stateTuple);
    events: for (int e = mFirstEvent; e <= mLastEvent; e++) {
      Arrays.fill(mNDTuple, null);
      for (final int a : mEventAutomata[e]) {
        if (mTransitions[a][e] != null) {
          final int[] succ = mTransitions[a][e][stateTuple[a]];
          if (succ == null) {
            continue events;
          }
          mNDTuple[a] = succ;
      ...
      permutations(mNumAutomata, stateTuple, state, e);
    }
  }

  public void permutations(int a, final int[] stateTuple, final int state,
                           final int event) throws OverflowException
  {
    if (a == 0) {
      processNewState(state, event,
                      stateTuple == null);
    } else {
      a--;
      ...
      if (codes == null) {
        ...
        permutations(a, stateTuple, state, event);
      } else {
  ...
  public abstract void processNewState(final int source, final int event,
                                       final boolean isInitial) throws OverflowException;

  private final int[][] mEventAutomata;
  private final int[][][][] mTransitions;
  private final int[][] mNDTuple;
  int mFirstEvent;
  int mLastEvent;
}


//# Inner Class InitialReachabilityExplorer
private class InitialReachabilityExplorer extends StateExplorer
{
  public InitialReachabilityExplorer(final int[][] eventAutomata,
                                     final int[][][][] transitions, final int[][] NDTuple, final int firstEvent, final int lastEvent)
  {
    super(eventAutomata, transitions, NDTuple, firstEvent, lastEvent);
  }

  public void processNewState(final int source, final int event,
                              final boolean isInitial) throws OverflowException
  {
    final int target;
...
private int[][][][] reverseTransitions;////
private int[][] eventAutomata;////
private int[][] reverseEventAutomata;////
private int[][] mNDTuple;////
private int[][] mNDTuple2;////
private int[][] mNDTupleRvs;////

private StateExplorer mInitialReachabilityExplorer;////
