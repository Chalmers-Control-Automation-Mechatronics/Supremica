...
public class CertainConflictsTraceExpander extends TRTraceExpander
{
  ...
  //#######################################################################
  //# Constructors
  protected CertainConflictsTraceExpander
    ...
    throws AnalysisException
  {
    ...
    l("rel has " + rel.getNumberOfReachableStates() + " states");
    ...
    l("rel has " + mCertainConfRel.getNumberOfReachableStates() + " states");
    ///////////////////////////////////////////
   ...
   // newTraceSteps = relabelInitialTraceSteps(newTraceSteps, stateMap);
    ...
    mCheckedProposition = factory.createEventProxy(":certainconf", EventKind.UNCONTROLLABLE);
    List<TraceStepProxy> additionalSteps = null;

    int level = initTest == -1 ? 0 : Math.max(0, mLevels[initTest]);
    l("initialising level to " + level);
   // int foundLevel = 0;

    do {
       l("Calling findNextLevel at level " + level);
      level = findNextLevel(initTest, level);
    ...
    final int state = getTestAutomatonEndState(newTraceSteps);
    l("final state: " + state);
