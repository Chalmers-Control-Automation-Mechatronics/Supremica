...
public class VerificationDialog implements ActionListener
{
   ...
   */
  public VerificationDialog(final Frame parentFrame,
                            ...
                            final MinimizationOptions minimizationOptions)
  {
    ...
    advancedPanelControllability =
      new VerificationDialogAdvancedPanelControllability();
    advancedPanelNonblocking =
      new VerificationDialogAdvancedPanelModularNonblocking();

    tabbedPane = new JTabbedPane();

    tabbedPane.addTab("Standard options", null, standardPanel,
                      "Standard options");
    tabbedPane.addTab("Advanced options", null, advancedPanelControllability,
                      "Advanced options");
  ...
  private class VerificationDialogStandardPanel extends JPanel
    implements VerificationPanel, ActionListener
  {
    ...
    {
      verificationTypeBox
        .setSelectedItem(verificationOptions.getVerificationType());
      algorithmSelector
        .setSelectedItem(verificationOptions.getAlgorithmType());
    ...
    private void updatePanel()
    {
      ...
      if ((verificationTypeBox
        .getSelectedItem() == VerificationType.CONTROLLABILITY
           || verificationTypeBox
             .getSelectedItem() == VerificationType.INVERSECONTROLLABILITY)
          && (algorithmSelector
            .getSelectedItem() == VerificationAlgorithm.MODULAR)) {
        ...
        tabbedPane.setComponentAt(advancedTabIndex,
                                  advancedPanelControllability);
        tabbedPane.setEnabledAt(advancedTabIndex, true);
      } else
        if (algorithmSelector
          .getSelectedItem() == VerificationAlgorithm.COMPOSITIONAL) {
      ...
      final VerificationAlgorithm selected =
        (VerificationAlgorithm) algorithmSelector.getSelectedItem();
      ...
      case LANGUAGEINCLUSION:
        addSelectable(VerificationAlgorithm.MONOLITHIC);
      ...
      case OP:
        ...
        break;
      case DIAGNOSABILITY:
        addSelectable(VerificationAlgorithm.BBSD);
        break;
      default:
        throw new IllegalStateException("Unknown verification type: " + vtype
                                        + "!");
      }
      // Default selection
      ...
      // Show trace?
      if (verificationTypeBox
        .getSelectedItem() == VerificationType.NONBLOCKING
          && algorithmSelector
            .getSelectedItem() == VerificationAlgorithm.MONOLITHIC
          || (verificationTypeBox
            .getSelectedItem() == VerificationType.CONTROLLABILITY
              || verificationTypeBox
                .getSelectedItem() == VerificationType.INVERSECONTROLLABILITY
              || verificationTypeBox
                .getSelectedItem() == VerificationType.LANGUAGEINCLUSION)
             && algorithmSelector
               .getSelectedItem() == VerificationAlgorithm.MODULAR) {
    ...
    private void updateNote()
    {
      ...
      case DIAGNOSABILITY:
        if (algorithmSelector
          .getSelectedItem() == VerificationAlgorithm.BBSD) {
    ...
    {
      verificationOptions
        .setVerificationType((VerificationType) verificationTypeBox
          .getSelectedItem());
      verificationOptions
        .setAlgorithmType((VerificationAlgorithm) algorithmSelector
          .getSelectedItem());
    ...
    private void addSelectable(final VerificationAlgorithm algo)
    {
      if (algo.isLoadable()) {
  ...
  class VerificationDialogAdvancedPanelModularNonblocking extends JPanel
  // implements MinimizationDialog.MinimizationPanel
  {
    ...
    public VerificationDialogAdvancedPanelModularNonblocking()
    {
      minimizationStrategy =
        new JComboBox<Object>(MinimizationStrategy.values());
      minimizationHeuristic =
        new JComboBox<Object>(MinimizationHeuristic.values());
    ...
    public void update(final MinimizationOptions options)
    {
      ...
      minimizationHeuristic
        .setSelectedItem(options.getMinimizationHeuristic());
    ...
    public void regain(final MinimizationOptions options)
    {
      ...
      options
        .setMinimizationStrategy((MinimizationStrategy) minimizationStrategy
          .getSelectedItem());
      options
        .setMinimizationHeuristic((MinimizationHeuristic) minimizationHeuristic
          .getSelectedItem());
  ...
  class VerificationDialogAdvancedPanelControllability extends JPanel
    implements VerificationPanel
  {
    ...
    public VerificationDialogAdvancedPanelControllability()
    {
      ...
      final JLabel exclusionStateLimitText =
        new JLabel("Initial state limit for state exclusion");

      exclusionStateLimit = new JTextField();

      final JLabel reachabilityStateLimitText =
        new JLabel("Initial state limit for reachability verification");

      reachabilityStateLimit = new JTextField();
      oneEventAtATimeBox =
        new JCheckBox("Verify one uncontrollable event at a time");
      skipUncontrollabilityBox =
        new JCheckBox("Skip uncontrollability check");

      final JLabel nbrOfAttemptsText =
        new JLabel("Number of verification attempts");
    ...
    public void update(final VerificationOptions verificationOptions)
    {
      exclusionStateLimit.setText(Integer
        .toString(verificationOptions.getExclusionStateLimit()));
      reachabilityStateLimit.setText(Integer
        .toString(verificationOptions.getReachabilityStateLimit()));
      oneEventAtATimeBox
        .setSelected(verificationOptions.getOneEventAtATime());
      skipUncontrollabilityBox
        .setSelected(verificationOptions.getSkipUncontrollabilityCheck());
      nbrOfAttempts
        .setText(Integer.toString(verificationOptions.getNbrOfAttempts()));
    }
    ...
    public void regain(final VerificationOptions verificationOptions)
    {
      ...
      verificationOptions.setExclusionStateLimit(Integer
        .parseInt(exclusionStateLimit.getText())); // Should have min and max values?
      ...
      verificationOptions.setReachabilityStateLimit(Integer
        .parseInt(reachabilityStateLimit.getText())); // Should have min and max values?
      verificationOptions.setOneEventAtATime(oneEventAtATimeBox.isSelected());
      verificationOptions
        .setSkipUncontrollabilityCheck(skipUncontrollabilityBox.isSelected());
      //verificationOptions.setNbrOfAttempts(PreferencesDialog.getInt("Nbr of attempts limit", nbrOfAttempts.getText(), 1));
      verificationOptions
        .setNbrOfAttempts(Integer.parseInt(nbrOfAttempts.getText()));
    }
  }
}
