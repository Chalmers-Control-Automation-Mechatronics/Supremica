...
/**
 * This is a first try of doing proper interleave. It turns out that this can
 * be done by using actions already available. It goes like this:
 * <OL>
 * <LI>Make all events unobservable (watch out for already unobservable
 * events, such as tau events!)</LI>
 * <LI>Synchronise ordinarily</LI>
 * <LI>Set the observability property back of all events touched under 1
 * above</LI>
 * <LI>Do language preserving minimisation</LI>
 * </OL>
 *
 * This is the worker that takes care of things once the synchronised result
 * has been added Note that it does not work the same way as the other
 * Automata*Worker, as this one is really only a listener, that reacts to what
 * it hears.
 *
...
class AutomataInterleaveWorker implements AutomataListener
{
  private static final Logger logger =
    LogManager.getLogger(AutomataInterleaveWorker.class);
  ...
  public AutomataInterleaveWorker(final IDEActionInterface ide)
  {
    this.IDE = ide;
    this.analyzerPanel =
      this.IDE.getActiveDocumentContainer().getSupremicaAnalyzerPanel();
    this.currentMode = SYNC_MODE;

    final Automata selectedAutomata =
      this.analyzerPanel.getSelectedAutomata();
    ...
    final AutomataSynchronizerWorker asw =
      new AutomataSynchronizerWorker(ide.getIDE(), selectedAutomata, "",
  ...
  {
    logger.debug("AutomataInterleaveWorker: Automaton " + automaton.getName()
                 + " added!");
    ...
    if (this.currentMode == SYNC_MODE) {
      ...
      final AutomataMinimizationWorker amw =
        new AutomataMinimizationWorker(IDE.getFrame(), selectedAutomata,
                                       analyzerPanel.getVisualProject(),
                                       options);
      amw.start();
    } else if (this.currentMode == MIN_MODE) {
      /*
       * We assume that with keepOriginal == false, we get notified in the
       * following order: synchronization result added in SYNC_MODE
       * synchronization result removed in MIN_MODE minimization result added
       * in MIN_MODE
       */
      ...
      final Automata selectedAutomata = new Automata();
      selectedAutomata.addAutomaton(automaton);
      final EnumerateStates enumerateStates =
        new EnumerateStates(selectedAutomata, "q");
      enumerateStates.execute();
    }
  }

  @Override
  public void automatonRemoved(final Automata automata,
                               final Automaton automaton)
  {
  ...
  @Override
  public void automatonRenamed(final Automata automata,
                               final Automaton automaton)
  {
  ...
  public void actionsOrControlsChanged(final Automata automata)
  {
    ...
    listeners.removeListener(this);
    logger
      .error("AutomataInterleaveWorker: actionsOrControlsChanged not expected");
  }
 ...
 * A new action
 */
public class AnalyzerExperimentAction extends IDEAction
{
  private static final Logger logger =
    LogManager.getLogger(AnalyzerExperimentAction.class);
  ...
  public AnalyzerExperimentAction(final List<IDEAction> actionList)
  {
    ...
    putValue(Action.SHORT_DESCRIPTION,
             "Test of new stuff - this time: interleave");
  ...
  /*
   * Try to synch the selected automata and then check that the new automaton
   * is there - does not work correctly
   */
  void interleaveExperiment() //-- MF
  {
    final Automata selectedAutomata = ide.getActiveDocumentContainer()
      .getSupremicaAnalyzerPanel().getSelectedAutomata();
    if (!selectedAutomata.sanityCheck(ide.getIDE(), 2, true, false, false,
                                      false)) {
      return;
    }
    ...
    /****************
     * Try to let the InterleaveListener do all the work final AnalyzerPanel
     * analyzerPanel = ide.getActiveDocumentContainer().getAnalyzerPanel();
     * final Automata allAutomata = analyzerPanel.getAllAutomata(); // This
     * really returns the visualProject // final int nbrAutomataBefore =
     * allAutomata.nbrOfAutomata(); final Automata selectedAutomata =
     * analyzerPanel.getSelectedAutomata();
     *
     * final Alphabet alpha = selectedAutomata.getUnionAlphabet(); final
     * Alphabet changed_alpha = new Alphabet(); // Here we store the ones we
     * change
     *
     * for(final LabeledEvent event : alpha) { if(event.isObservable()) // If
     * this event is obs, set it un-obs and remember to set it back {
     * event.setObservable(false); changed_alpha.add(event); } }
     *
     * // Add the listener that is to do the work once the synchronized
     * automaton has been added final AutomataListener inlisten = new
     * InterleaveListener(ide, analyzerPanel, changed_alpha);
     * allAutomata.addListener(inlisten);
     *
     * // Now synchronize final SynchronizationOptions synchronizationOptions
     * = new SynchronizationOptions(); // Make sure unobs non-tau events do
     * NOT synch! final AutomataSynchronizerWorker asw = new
     * AutomataSynchronizerWorker(ide.getIDE(), selectedAutomata, "",
     * synchronizationOptions);
     *
     * asw.start(); // Start the synch thread and wait for it to finish
     **********************************************************************/
    /**************
     * We don't need this, just let asw roam free and we get informed when the
     * synch result has been added while(asw.isAlive()) { try { asw.join(); //
     * Wait for it to finish } catch (InterruptedException excp) // May be
     * thrown by join (and sleep, and wait) { // Apparently this is the way to
     * do it // See
     * http://www.yegor256.com/2015/10/20/interrupted-exception.html
     * Thread.currentThread().interrupt(); throw new RuntimeException(excp); }
     * }
     *************************************************************/
    /********************************
     * this is not the way to do it! // Now, somehow get the newly created
     * automaton, the synch result // For now we just check that something was
     * added, as we expect final int nbrAutomataAfter =
     * analyzerPanel.getAllAutomata().nbrOfAutomata(); // The diff between
     * nbrAutomataBefore and nbrAutomataAfter should be a single automaton,
     * the one we look for final int num = nbrAutomataAfter -
     * nbrAutomataBefore; System.out.println("Num all automata before: " +
     * nbrAutomataBefore); System.out.println("Num all automata after: " +
     * nbrAutomataAfter); if(num != 1) { // Something is seriously wrong,
     * either no automaton or more than one was created by the synch...
     * logger.error("Experiment error! " + num + " number of automata was
     * created, we expected one, and only one!"); }
     *
     * // The above does not work correctly, the automaton is added by the
     * synch worker, but apparently AFTER // the join() has happened, so the
     * num of automata before and after are always(?) the same // Probably
     * some race condition - have to rethink this
     ********************************/
  }

  /*
   * So justAddAutomatonExperiment finally works as expected. Now let's try
   * the threaded variant Also this seems now to work fine
   */
  void addAutomatonThreadedExperiment() //-- MF
  {
    final Automata allAutomataBefore = ide.getActiveDocumentContainer()
      .getSupremicaAnalyzerPanel().getAllAutomata();
    final int nbrAutomataBefore = allAutomataBefore.nbrOfAutomata();

    final AnalyzerAddAutomatonJustForExperiment adder =
      new AnalyzerAddAutomatonJustForExperiment(ide);
    adder.start();
    ...
    final Automata allAutomataAfter = ide.getActiveDocumentContainer()
      .getSupremicaAnalyzerPanel().getAllAutomata();
    ...
    System.out
      .println("Num all automata after: " + allAutomataAfter.nbrOfAutomata());
    if (num != 1) {
      // Something is seriously wrong, either no automaton or more than one was created...
      System.out
        .println("Experiment error! " + num
                 + " number of automata was created, we expected one, and only one!");

    }
  }

  /*
   * Something is not working as expected when it comes to waiting for the
   * worker thread to finish using join(), so this is to run some simple test
   * to try to determine how this whole thing should work. It turns out that
   * it is the getAllAutomata() that does not work as expected! It does not
   * give you a list of all Automata, it gives you a reference to the
   * visualProject, which means that in the code below, allAutomataBefore and
   * allAutomataAfter refer to the exact same object, and hence the
   * nbrOfAutomata will be the same when checked at the end! This was tricky!!
   */
  void justAddAutomatonExperiment() //-- MF
  {
    ...
    /*
     * Does it work if we just add the automaton here without starting that
     * other thread?
     */
    System.out.println("Adding automaton...");
    final Automaton new_auto = new Automaton("Mxyzptlk");
    final boolean done = ide.getActiveDocumentContainer()
      .getSupremicaAnalyzerPanel().addAutomaton(new_auto);
    System.out.println("Added automaton: " + done);
    /*
     * NO! It still doesn't work. done is true, but num below still becomes 0!
     * WTF?
     */
    /*
     * But OK, so this means this is not a racing issue, but it simple does
  ...
  /*
   * **************************************************************** This is
   * some kind of "split", unclear who wrote it but apparently it was not good
   * enough to get a menu option of its own... void splitExperiment() { //
   * "DECOMPOSE" INDIVUDUAL AUTOMATA for (final Automaton automaton:
   * selectedAautomata) { final Automata result =
   * AutomatonSplit.split(automaton);
   *
   * try {
   * ide.getIDE().getActiveDocumentContainer().getAnalyzerPanel().addAutomata(
   * result); } catch (final Exception ex) {
...
/**
 * These were used for all kinds of experimentation when I (MF) thought that
 * the bug was in my code when it really was in
 * AnalyzerPanel.getAllAutomata().
 */
class AnalyzerAddAutomatonJustForExperiment extends Thread
  implements Abortable
{
  @SuppressWarnings("unused")
  private static final Logger logger =
    LogManager.getLogger(AnalyzerAddAutomatonJustForExperiment.class);

  private Object mAbortRequester;
  @SuppressWarnings("unused")
  private final IDEActionInterface IDE;
  private final SupremicaAnalyzerPanel analyzerPanel;
...
class AnalyzerInterleaveWorker extends Thread implements Abortable
{
  ...
  public void run()
  {
    final int nbrAutomataBefore =
      analyzerPanel.getAllAutomata().nbrOfAutomata();
    ...
    // Now synchronize
    final SynchronizationOptions synchronizationOptions =
      new SynchronizationOptions(); // Make sure unobs non-tau events do NOT synch!
    final AutomataSynchronizerWorker asw =
      new AutomataSynchronizerWorker(IDE.getIDE(), selectedAutomata, "",
                                     synchronizationOptions);
    if (asw != null) {
    ...
    if (num != 1) {
      ...
      throw new RuntimeException("Experiment error! " + num
                                 + " number of automata was created, we expected one, and only one!");
    }
