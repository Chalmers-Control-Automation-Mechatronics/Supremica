    ...
    extends EditorSavePostscriptAction
{
    ...
    public void doAction()
    {
        ...
        try
        {
            EditorSurface surface = ide.getActiveDocumentContainer().getEditorPanel().getActiveEditorWindowInterface().getControlledSurface();
            graph = surface.getGraph();
            module = surface.getModule();
            name = ide.getActiveDocumentContainer().getEditorPanel().getActiveEditorWindowInterface().getComponentName();
        }
        catch (NullPointerException ex)
        {
            // This action should only be enabled when theres an editor panel open!
            ide.info("Must have an editor panel open.");
            return;
        }
        ...
        try
        {
            ...
            chooser.setSelectedFile(new File(name + ".eps"));
            int returnVal = chooser.showSaveDialog(ide.getIDE());
                               ...
                               final ModuleProxy module)
    {
        ...
        try {
            ...
            ProxyShapeProducer producer = new ProxyShapeProducer(graph, module);
            producer.createAllShapes();
