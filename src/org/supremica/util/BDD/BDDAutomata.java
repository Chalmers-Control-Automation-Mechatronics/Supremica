    ...
    extends JBDD
{
	...
	Assert.assert((not(getZero()) == getOne()) && (not(getOne()) == getZero()),
	...
	while(e.hasMoreElements()) {
	    ...
	    //Assert.debug(automata[i-1].getName() + " created");
	}
	...
	keep              = getOne();   ref(keep);
	bdd_total_initial = getOne();   ref(bdd_total_initial);
	bdd_total_cube    = getOne();   ref(bdd_total_cube);
	bdd_total_cubep   = getOne();   ref(bdd_total_cubep);
	...
	Assert.assert(!has_events, "[BDDAutomata.createEvents] multiple calls"+
		      " to createEvents!");
	...
	bdd_events_u    = getZero(); ref(bdd_events_u);
	bdd_events_c    = getZero(); ref(bdd_events_c);
	...
	for(int i = 0; i < events_size; i++) {
	    ...
	    if(!original_events[i].c) bdd_events_u = orTo(bdd_events_u, bdd_e);
	    else                      bdd_events_c = orTo(bdd_events_c, bdd_e);

	    // Assert.debug("event_i: " + original_events[i].name + "/" +original_events[i].code);
    ...
    public int getSize() { return components; }
    public int getEventCube() { return bdd_events_cube; }
    public int getEventsBits() { return size_events; }


    public int getStateCube() { return bdd_total_cube; };
    public int getStatepCube() { return bdd_total_cubep; }
    public int getStateBits() { return size_states; }

    public int getSigmaU() { return bdd_events_u; }
    public int getSigmaC() { return bdd_events_c; }

    public int getPermuteS2Sp() { return permute_s2sp; }
    public int getPermuteSp2S() { return permute_sp2s; }

    public Event [] getEvents() {
	Assert.assert(has_events,"[BDDAutomata.getEvents] call createEvents firs!");
	...
	ps.println("BDD Sigma_u: " + nodeCount(bdd_events_u) + " nodes, SAT-count = " +
		   satCount(bdd_events_u, size_events));
    }

    // ------------------------ some debugging functions ----------------------------------

    // --------------------------------------------------------- show_states (note: O(n!) complexity)
    public void show_states(int bdd) { show_states(bdd, null); }
	...
	if(level >= components) {
	    System.out.print(" < ");
	    for(int i = 0; i < components; i++)
		if(saved[i] != null) System.out.print(saved[i] + "  ");
	    System.out.println(">");
    ...
    public void printStateVector(String [] states) { printStateVector(states, null); }
    public void printStateVector(String [] states, String what) {

	if(what != null) System.out.print(what);

	System.out.print(" <");
	for(int i = 0; i < states.length; i++) {
	    if(i != 0) System.out.print(", ");
	    System.out.print(states[i] == null ? "-" : states[i]);
	}
	System.out.println(">");
    }
    // -----------------------------------------------------------------------------
    public int pickOneState(String [] names, int bdd) {
	if(bdd == getZero()) return bdd; // nothing to choose
	...
	for(int i = 0; i < states.length; i++) {
		...
		if(names != null)  names[level] = states[i].name;
		recursiveDeref(tmp);
		return ret;		
	    }
	    recursiveDeref(tmp);	    
	...
	*/
    ...
    private void count_transitions_rec0(Counter c, int bdd, int level) { // for S
	if(level >= components) {
	    c.increase();
	    return;
	}
    ...
    private void show_transitions_rec0(String [] names, int bdd, int level) { // for S
	if(level >= components) {
    ...
    private void show_transitions_rec1(String [] names, int bdd, int level) { // for S'
	if(level >= components) {
    ...
    private void show_transitions_rec2(String [] names, int bdd) { // and for Sigma
    ...
    private void show_transitions_print(String [] list) { // pretty printer for the transition list
	int i;
	System.out.print("  ( <");
	...
	for(i = 0; i < components; i++){
	    if(i != 0) System.out.print(",");
	    if(list[components + i] != null) System.out.print(list[components + i]);
	}
	System.out.println("> ), ");
    }
	...
	if(!Options.sanity_check_on) return; // ignore function call

	// DEBUG: in case checkPackage crashes before it exists and prints 'name'
	System.out.println("Checking : " + name);

	Assert.assert(super.checkPackage(), name + ": checkPackage() failed");
	...
	for(int i = 0; i < components; i++){
	    ...
	    else       Assert.warning("Automata " + automata[i].getName() + " has no states!");	    
	}
	*/
	...
	Assert.debug("Prioritized composition");
	// System.out.println("T-all = "); printSet(t_all);
	// show_states(t_all, null);
	Assert.debug("|prioritizied T-all| = " + nodeCount(t_all));
	...
	*/
	...
	do {
		...
		recursiveDeref(front); front = tmp2;
	    }
