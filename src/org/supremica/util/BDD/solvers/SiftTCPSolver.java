...
public class SiftTCPSolver extends TSPSolver {
	private Node [] work = null;
	public int improvments;
	public SiftTCPSolver(Node [] org_ ) {	super(org_); }


	public void solve() {
		super.solve();

		if(size < 2) return; // no need to wast time here


		work = new Node[size];
		for(int i = 0; i < size; i++) work[i] = solved[i]; // start with our TSP solution
		...
		for(int j = 0; j < size * 5; j++) {
			...
			if(score < best) {
				...
				// change it back!
				tmp = work[r]; work[r] = work[to]; work[to] = tmp;
			}
		}
		...
		if(Options.profile_on) {
			Options.out.println("TCP+sift algorithm " +
				(improvments  > 0 ?
