	...
	private class KeepSmoothPartition {
		...
		private class KeepCluster {
			...
			public KeepCluster(BDDAutomata manager, Cluster cluster) {
				...
				for(Enumeration e = cluster.members.elements(); e.hasMoreElements() ; ) {
				...
				for(Enumeration e = cluster.members.elements(); e.hasMoreElements() ; ) {
					...
					int [] vp = a.getVar();
					int [] v = a.getVarp();
					for(int i = 0; i < size; i++) v_vars[bits++] = manager.and(v[i], vp[i]);
				}
			...
			public void cleanup() {
				...
				if(bdd_curr_t != -1) manager.deref(bdd_curr_t);
				for(int i = 0; i < bits; i++) manager.deref(v_vars[i]);
			}
