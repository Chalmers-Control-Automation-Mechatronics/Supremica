...
public class PCG {
	...
	Assert.assert(node1 != null, "first object not found");
	Assert.assert(node2 != null, "second object not found");
	...
	Assert.assert(!first, "[PCG.getSmallest] free-list is empty");
	return index;
    }
	...
	if(size <= 1) { // 0 or 1 components
	    perm = new int[size];
	    if(size == 1) perm[0] = 0; // the one and only	    
	    return perm;
	}
	...
	if(Options.user_alters_PCG) { // user can change the ordering??
	    ...
	    if(cost_before > cost_after) System.out.print("Ordering improved by user");	    
	    else if(cost_before < cost_after) System.out.print("Ordering weaken by user");
	...
	for(int i = 0; i < size; i++) free[i] = true;
	...
	if(level == size) {
		...
		*/
		...
		for(int i = 0; i < size; i++) perm[i] = path[i];
		perm[size] = cost;
	    }
	    return ;
	}
	...
	Assert.assert(index != -1, "1: INDEX == -1");
	...
	Assert.assert(index != -1, "2: INDEX == -1");
	return index;
    }
