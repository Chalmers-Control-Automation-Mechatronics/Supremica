	...
	extends BaseLI
{
	...
	private void bdd_cleanup()
	{
		...
		if (bdd_local_events_plant != -1)
		{
			ba.deref(bdd_local_events_plant);

			bdd_local_events_plant = -1;
		}
	...
	public boolean check(BDDAutomaton Xi, Coverage ac)
	{
		...
		while ((next = ac.next()) != null)
		{
			...
			if (Options.debug_on)
			{
				Options.out.println("\t +++ Adding " + next.getName() + "\n");
			}
			...
			if (changed > 0)
			{
				...
				if (Options.debug_on)
				{
					Options.out.println("*** Removed " + changed + " events from workset-events.");
				}
			}
			...
			if (last_plant_locals < new_plant_locals)
			{
				...
				if (Options.debug_on)
				{
					ba.getEventManager().dumpSubset("*** IncrLC:(" + (new_plant_locals - last_locals) + " new) PlantLocalEvents", plocals);
				}
			...
			if (last_locals < new_locals)
			{
				...
				if (Options.debug_on)
				{
					ba.getEventManager().dumpSubset("*** IncrLC:(" + (new_locals - last_locals) + " new) LocalEvents", locals);
				}
	...
	private void update_theta(BDDAutomaton next, boolean is_spec)
	{
		if (controllaibilty_test)
		{
			...
			else
			{
				...
				int tmp1 = ba.exists(next.getTpri(), bdd_cube_sp);
				int tmp2 = ba.and(tmp1, bdd_sigma_w);

				ba.deref(tmp1);

				bdd_theta_j = ba.andTo(bdd_theta_j, tmp2);
		...
		else
		{
			...
			bdd_theta = ba.andTo(bdd_theta, tmp2);
