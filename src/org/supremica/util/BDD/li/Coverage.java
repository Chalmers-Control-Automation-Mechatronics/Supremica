...
public class Coverage
{
	...
	/** num of local events */
	private int num_locals_plant;

	/** num of local events */
	private boolean[] relevant_plant_events_remain;

	/** events that should be added before plant has all the relevant events */
	private boolean[] local_events;

	/** local events */
	private boolean[] local_plant_events;

	/** local events in the plant */
	private int[] event_usage_plant_only;

	/** Event usage, but only for plants */
	private int[] event_usage_reminas;

	/** number of times this event has been in the "rest" of the system*/
	...
	public Coverage(BDDAutomaton target, BaseLI li)
		throws TrivialPass
	{
		this.li = li;
		this.target = target;
		...
		for (int i = 0; i < size; i++)
		{
			if ((li.all[i] != target) &&!in_use[i] && (li.controllaibilty_test ||!li.is_spec[i]))
			{
				if (li.all[i].interact(li.considred_events))
				{
					...
					if (Options.debug_on)
					{
						Options.out.println("  \tLC: added automaton " + li.all[i].getName() + ", type: " + (li.is_spec[i]
																											 ? "Spec"
																											 : "Plant")
	...
	// ----------------------------------------------------------------------
	public boolean lastWasSpec()
	{
		return last_was_spec;
	}

	public boolean[] getLocalEvents()
	{
		return local_events;
	}

	public boolean[] getLocalPlantEvents()
	{
		return local_plant_events;
	}

	public int getNumOfLocalEvents()
	{
		return num_locals;
	}

	public int getNumOfLocalPlantEvents()
	{
		return num_locals_plant;
	}
