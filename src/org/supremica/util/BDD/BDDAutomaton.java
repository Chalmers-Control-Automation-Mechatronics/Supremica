...
public class BDDAutomaton {
	...
	bdd_m = manager.getZero(); manager.ref(bdd_m);
	bdd_f = manager.getZero(); manager.ref(bdd_f);

	bdd_care_s  = manager.getZero(); manager.ref(bdd_care_s);
	bdd_care_sp = manager.getZero(); manager.ref(bdd_care_sp);
	...
	for(int i = 0; i < num_states; i++) {
	    ...
	    if(states[i].m) bdd_m = manager.orTo(bdd_m, bdd_s);
	    if(states[i].f) bdd_f = manager.orTo(bdd_f, bdd_sp);
	...
	bdd_events   = manager.getZero(); manager.ref(bdd_events);
	bdd_events_p = manager.getZero(); manager.ref(bdd_events_p);
	bdd_events_u = manager.getZero(); manager.ref(bdd_events_u);
	...
	*/	
    ...
    // ------------------------------------------------------------------
    public int [] getVar() { return var_s; }
    public int [] getVarp(){ return var_sp; }
    public int    getCube() { return cube_s; }
    public int    getCubep() { return cube_sp; }

    public int getI() { return bdd_i; }     // I(s)
    public int getT() { return bdd_t; }     // T(s,s',e)
    public int getTu() { return bdd_tu; }   // Tu(s,s',e) , e in Sigma_u
    public int getTtop() { return bdd_t_top; }  // T^(s,s',e) , see Knuts "A note on prioritized sync..."
    public int getTpri() { return bdd_t_pri; } // T prioritized, seeKnuts paper again

    public int getM() { return bdd_m; }     // M(s)
    public int getF() { return bdd_f; }     // X(s) or F(s) (forbidden)

    public int getCareS() { return bdd_care_s; }   // subset of S, only states that are used
    public int getDontCareS() { return bdd_dontcare_s; }// S \ care-S, i.e. states not used
    
    public int getCareSp() { return bdd_care_sp; }   
    public int getDontCareSp() { return bdd_dontcare_sp; }

    public int getSigmaP() { return bdd_events_p; }  	// Sigma_p(e) - prioritized events
    public int getSigma() { return bdd_events; } // Sigma(e) - all events used in this automaton
    public int getSigmaU() { return bdd_events_u; } // Sigma_u(e) - uncontrollable events
    
    public int getKeep() { return bdd_keep; }
    
    public int getPermuteS2SP() { return permute_s2sp; }
    public int getPermuteSP2S() { return permute_sp2s; }

    public State [] getStates() { return states; } // the non-symbolic state set + thier BDD coding and more...
    public String getName() { return automaton.getName(); }
    public Automaton getModel() { return automaton; }
    public int getType() { return automaton.getType(); }

    public int getNumStates() { return num_states; }
    public int getNumStateBits() {return num_bits; }

    // ------------------------------------------------------------------
    private void createI() {
	bdd_i = manager.getOne();  manager.ref(bdd_i);	
	for(int i = 0; i < num_states; i++) {
	    if(states[i].i) {
	...
	bdd_tu = manager.getZero(); manager.ref(bdd_tu);
	bdd_t  = manager.getZero(); manager.ref(bdd_t);
	...
	for(int i = 0; i < num_arcs; i++) {
	    ...
	    if(uncontrollable)	 bdd_tu = manager.orTo(bdd_tu, this_t);
	...
	manager.recursiveDeref(bdd_t_top);	bdd_t_top = tmp;
	...
	manager.recursiveDeref(not_sigma_p); bdd_t_pri = tmp;

	tmp = manager.exists(bdd_t_pri, cube_sp); 
	manager.recursiveDeref(bdd_t_pri); bdd_t_pri = tmp;
	
	tmp = manager.and(bdd_t_top, bdd_t_pri);
	manager.recursiveDeref(bdd_t_pri); bdd_t_pri = tmp;


	// Assert.debug("BDD_T"); manager.printSet(bdd_t);
	// Assert.debug("BDD_T_TOP");	manager.printSet(bdd_t_top);
	// Assert.debug("BDD_T_PRI"); manager.printSet(bdd_t_pri);
    }

    // -------------------------------------------------------------------
    private void dumpBDD(PrintStream ps, String name, int bdd, int vars) {
	ps.println("BDD " + name + ": " + manager.nodeCount(bdd) + " nodes, SAT-count = " +
		   manager.satCount(bdd,vars) + ", refcount = " +
		   manager.internal_refcount(bdd));
    }
    ...
    private void check(String name) { // for debug only: check automaton integrity:

	if(! Options.sanity_check_on) return; // ignore
	...
	if(errors > 0) {
	    System.err.println("TEST '" + name +
			       "' --> BDD structure integrity check failed for: " +
			       automaton.getName());
	    System.exit(20);
	}
    

	Assert.assert(manager.checkPackage(), name + ": checkPackage() failed");
    }
