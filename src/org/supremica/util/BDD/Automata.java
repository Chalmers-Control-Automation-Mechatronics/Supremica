...
public class Automata {
	...
	Assert.assert(!closed, "[Automata.createAutomaton] BAD function call");
	current = new Automaton(name,alphabet);
	automata.addElement(current);
	return current;
    }

    public Automaton getCurrent() {	return current;    }
    public EventManager getAlphabeth() { return alphabet; }


    public double getTotalSize() { return total_size; }
    public double getTotalSizeAllocated() { return total_size_allocated; }
    public int getVariableCount() { return variable_count; }

    public void close() {
	Assert.assert(!closed, "[Automata.close] BAD function call");
	alphabet.close();
	...
	for(int i = 0; i < components; i++) { // get weights
	...
	ps.println("" + automata.size() + " automata ("
		   + (long)total_size + " states max, " 
		   + (long)total_size_allocated + " allocated)");
    }
}
