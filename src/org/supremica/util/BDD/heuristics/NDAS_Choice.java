...
{
	private static final int ADD_REWARD = +2, ADD_PUNISH = -1,    // tabu search magic number?
							 ADD_REWARD_FADING = +5;
	...
	public int choose(int[] queue, int size)
	{
		...
		if (size == 1)
		{
			return queue[0];    // XXX: this will skip any extra herusitic code below!
		}

		switch (ndas)
		{

		case Options.NDAS_FIRST :
			return queue[0];

		case Options.NDAS_LAST :
			return queue[size - 1];

		case Options.NDAS_RANDOM :
			return queue[(int) (Math.random() * size)];

		// case Options.NDAS_RING:              return queue[ ring++ % size ];
		case Options.NDAS_RING :
			return ring_choice(queue, size);

		case Options.NDAS_RL :
	...
	private int ring_choice(int[] queue, int size)
	{
		...
		if (ring == -1)
		{    // no smaller, start from the beginning
