...
public class ASH_Interactive extends AutomatonSelectionHeuristic  {
	...
	public int pick(int queue_size) {
		ic.removeAll();
		for(int i = 0; i < queue_size ; i++)  ic.add( list[ queue[i]].getName() );
		ic.show();
		int sel = ic.getSelected();

		if(sel == -1) return super.pick(queue_size);
