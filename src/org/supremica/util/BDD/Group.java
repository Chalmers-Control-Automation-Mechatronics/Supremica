...
public class Group {
	...
	bdd_m       = manager.getOne();  manager.ref(bdd_m);
	bdd_cube    = manager.getOne();  manager.ref(bdd_cube);   
	bdd_cubep   = manager.getOne();  manager.ref(bdd_cubep);
	bdd_sigma_u = manager.getZero(); manager.ref(bdd_sigma_u);
	...
	Assert.assert(size < capacity, "[Group.add] Group size exceeded");
	members[size] = a;
	size++;
    ...
    public BDDAutomaton [] getMembers() { return members; }
    
    public int getI() { return bdd_i; }
    public int getM() { return bdd_m; }
    public int getSigmaU() { return bdd_sigma_u; }
    public int getCube() { return bdd_cube; }
    public int getCubep() { return bdd_cubep; }
