	...
	extends Frame
{
	...
	public OnlineBDDSupervisor(BDDAutomata ba, int safe_states) throws Exception {
		super("OnlineBDDSupervisor");

		this.ba = ba;
		this.safe_states = safe_states;
		this.state_vector = new org.supremica.util.BDD.State[ba.getSize() ];
		for(int i = 0; i < state_vector.length; i++) state_vector[i] = null;
	...
	private void setup_mapping() throws Exception {
		amap = new HashMap();
		smap = new HashMap();
