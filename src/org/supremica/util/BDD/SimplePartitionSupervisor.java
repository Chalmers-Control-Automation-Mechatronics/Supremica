...
public class SimplePartitionSupervisor extends ConjSupervisor {
    ...
    public void cleanup() {
		for(int i = 0; i < t_wave.length; i++) manager.deref( t_wave[i]);
	...
	public  String type() { return "SimplePartitionSupervisor"; }
	...
	public int getReachables(boolean [] events, int intial_states) {
		...
		for(int i = 0; i < size; i++) manager.deref( partial_twave[i] );
		return x;
	}
