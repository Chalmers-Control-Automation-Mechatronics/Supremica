...
public class AutomataCover {
	...
	private int local_count, notused_count, shared_count, bdd_t, bdd_cubes_me, bdd_cubes_rest, bdd_local_t;
	private int [] eventUsageCount;
	private boolean [] eventIsLocal, eventCare, automataCover;
	private boolean has_local_t;
	private int size; /** number of automata in this cover */
	...
	public AutomataCover(BDDAutomata manager) {
		...
		size = 0;
		bdd_t = manager.getOne();			manager.ref(bdd_t);
		bdd_cubes_me = manager.getOne();	manager.ref(bdd_cubes_me );
		bdd_cubes_rest = manager.getStateCube();	manager.ref(bdd_cubes_rest );
		...
		for(int i = 0; i < events; i++) eventCare [i] = false;
		for(int i = 0; i < all.length; i++) automataCover [i] = false;
...
*/
	...
	public int getLocalCount() { return local_count - notused_count; }
	public int getNotUsedCount() { return notused_count; }
	public int getSharedCount() { return shared_count; }
	...
	private boolean updateLocalEvents() {
		...
		for(int i = 0; i < len; i++) {
			...
			if(eventCare[i] && ! eventIsLocal [i]) shared_count++;
		}
	...
	private int getLocalT() {
		if(!has_local_t) computeLocalT();
		return bdd_local_t;
	}
	private void computeLocalT() {
		int events = manager.getAlphabetSubsetAsBDD(eventIsLocal);
		int old_local_t, local_t = manager.relProd(bdd_t, events, manager.getEventCube() );
	...
	public BDDAutomaton queue_pop() { if(queue_size == 0) return null; queue_size--; return all[queue[queue_size]]; }
		...
		for(int i = 0; i < len; i++) {
			...
			if(!include(automaton)) {
				...
				if(overlap > 0) {
					...
					if(max > queue_costs[queue_size]) max = (int)queue_costs[queue_size];
					queue_size++;
				}
			}
		}
