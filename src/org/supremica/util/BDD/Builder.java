...
public class Builder {
	...
	if(name == null) return Automaton.TYPE_UNKNOWN;
	if(name.equals("plant"))   return Automaton.TYPE_PLANT;
	if(name.equals("specification"))   return Automaton.TYPE_SPEC;
	if(name.equals("supervisor"))   return Automaton.TYPE_SUPERVISOR;
	...
	for(Iterator it = s_automata.iterator(); it.hasNext(); ) {
	    ...
	    */
		...
		if(name == null || name.length() == 0) name = name_id;
		else if(name_id == null || name_id.length() == 0) name_id = name; // just for fun :)
		...
		// Insert ARCS
		for(EventIterator eit = s_a.outgoingEventsIterator(s_st); eit.hasNext(); ) {
