...
public class Supervisor {
    ...
    private Timer timer;
    private boolean has_uncontrollables, has_reachables, has_reachable_uncontrollables, has_coreachables;
    private int bdd_uncontrollables, bdd_reachables, bdd_reachable_uncontrollables, bdd_coreachables;
	...
	for(int i = 0; i < automata.length; i++) {
	    if(automata[i].getType() == Automaton.TYPE_SPEC) spec.add(automata[i]);
	    else if(automata[i].getType() == Automaton.TYPE_PLANT) plant.add(automata[i]);
	}
    ...
    public BDDAutomata getManager() { return manager; }
    public Group getP() { return plant; }
    public Group getSp() { return spec; }

    // ------------------------------------------------------------------------------

    public int getUncontrollableStates() {	
	if(!has_uncontrollables) computeUncontrollables();
	...
	*/
	...
	manager.recursiveDeref(tmp2); manager.recursiveDeref(cube2);
    ...
    */
    ...
    public int getReachables() {    // get reachables from I
	if(!has_reachables) computeReachables();
	...
	*/
	...
	if(!has_reachable_uncontrollables)  computeReachableUncontrollables();
    ...
    // ---------------------------------------------------------------------
    public int getCoReachables() {    // get CoReachables from M
	if(!has_coreachables) computeCoReachables();
	return bdd_coreachables;
    }
    private void computeCoReachables() {
	GrowFrame gf = null;;
	if(Options.show_grow) gf = new GrowFrame("backward reachability");
	...
	GrowFrame gf = null;
	if(Options.show_grow)  gf = new GrowFrame("Safe states: nodeCount(X)");
	...
	do {	    
	    ...
	    if(gf != null) gf.add( manager.nodeCount(x));
