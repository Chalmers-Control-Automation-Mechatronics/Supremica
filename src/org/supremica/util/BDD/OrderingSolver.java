...
public class OrderingSolver {
	...
	public void solve() {
	    ...
	    double best_cost = 0; // cost of splitting an arc, less is good
	    Vector candidates = new Vector(); // the set of those with smallest split-cost
		...
		// System.err.println("Candidates: " + candidates.size() + ", best: " + best_cost);
	    }
		...
		Integer integer = (Integer) candidates.elementAt(i);
		int b = integer.intValue();
		// System.out.println("--> " + b + "/"+ org[b].org.getName());
		int siz = org[b].size;
		if(i == 0 || siz < smallest) {
		    smallest = siz;
		    start_index = b;
		    // System.err.println("Good size: " + siz);
		}
	    }
	...
	public void insert(Node n, int [] edges, int len) {
	    ...
	    /*
	    // DEBUG:
	    for(int i = 0; i < edges.length; i++) System.out.print(edges[i] + " ");
	    System.out.println();
	    */

	    // mark that we just changed something:
	    localnodes = null;
	...
	private void sortClasses() { // bubblesort should do...
