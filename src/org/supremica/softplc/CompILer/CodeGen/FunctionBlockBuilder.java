	...
	extends ProgramAndFBBuilder
{
	...
	public FunctionBlockBuilder(String functionBlockName)
	{
		...
		mgRunArgs = new MethodGen(Constants.ACC_PUBLIC, Type.VOID, Type.NO_ARGS, null, "runArgs", className, ilRunArgs, constPoolGen);
		...
		// varför behöver man owner???? Anders undrar vad han tänkt på:(
	...
	public void emitVarField(String varName, Object type, boolean global, boolean inputOutputVar)
	{
		...
		/*
		 * Det mesta av koden här bör kanske läggas i en egen class
		 * för field-generering(global variables are only allowed at
		 *               configuration, resource or program level)
		 */
		if (global)
		{
			System.err.println("Global variables not allowed in function " + "blocks");
	...
	{
		System.err.println("Reference to direct variable not allowed " + "in function blocks");
