...
public class ProgramBuilder extends ProgramAndFBBuilder {
	private String[] programInterfaces =
		{"org.supremica.softplc.CompILer.CodeGen.IEC_Interfaces.IEC_Program"};
    ...
    public ProgramBuilder(String programName){
		...
		FieldGen var = new FieldGen (Constants.ACC_PRIVATE,
		...
		var = new FieldGen (Constants.ACC_PRIVATE,
							new ArrayType(Type.BOOLEAN,1),
        ...
        mgInit = new MethodGen(Constants.ACC_PUBLIC, Type.VOID,
                               new Type[]{new ArrayType(Type.BOOLEAN,1),
                                          new ArrayType(Type.BOOLEAN,1)},
                               new String[] {"inputSignals","outputSignals"},
                               "<init>",className,ilInit,constPoolGen);
	...
	public void emitDirectInit(IECDirectVariable v, TypeBOOL i) {
		if (v.isInput()) {
			System.err.println("Initialisation of direct input variables "+
							   "not allowed.");
		}
		else {
	...
	InstructionList emitLoadVariable(IECDirectVariable var) {
		...
		/*		if (!var.isInput()) {
		 *System.err.println(var + " is write only");
		 *errorsPresent = true;
		 *}
		 *else {
		 */
