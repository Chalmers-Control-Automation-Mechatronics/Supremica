	...
	extends ProgramAndFBBuilder
{
	...
	InstructionList emitLoadVariable(IECDirectVariable var)
	{
		...
		if (type == TypeConstant.T_BOOL)
		{
			...
			il.append(fac.createFieldAccess(className, directInput, new ArrayType(Type.BOOLEAN, 1), Constants.GETFIELD));
			il.append(new PUSH(constPoolGen, nr));    // ladda pos
			il.append(fac.createArrayLoad(Type.BOOLEAN));
		}
		else
		{
			System.err.println("Loading direct variables of type " + type + " not yet implemented");
	...
	InstructionList emitStoreVariable(IECDirectVariable var)
	{
		...
		else
		{
			...
			{
				System.err.println("Storing direct variables of type " + type + " not yet implemented");
			}
		}
