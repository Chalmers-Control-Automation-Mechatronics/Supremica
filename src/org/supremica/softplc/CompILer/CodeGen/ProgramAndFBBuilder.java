	...
	implements Builder
{
	...
	public void emitVarField(String varName, Object type, boolean global, boolean inputOutputVar)
	{
		...
		{
			error("Variables of this kind " + type.toString() + "is not implemented.");
		}
	}
	...
	public void emitVarField(String varName, TypeANY_ELEMENTARY type, boolean global, boolean inputOutputVar)
	{
		...
		{
			error("Initialised variables of type " + type.getType() + " not implemented." + "Try manual initialisation.");
		}
	}
	...
	InstructionList emitLoadVariable(IECSymbolicVariable var)
	{
		...
		else if (type == TypeConstant.T_DERIVED /*including function blocks*/)
		{
			...
			{    /* we have got a variable in a derived variable */
				...
				if (type == TypeConstant.T_BOOL)
				{
					il.append(fac.createFieldAccess( /*fb type name*/var.getTypeName(), var.getFieldSelector(), Type.BOOLEAN, Constants.GETFIELD));
				}
				else if (type == TypeConstant.T_DINT)
				{
					il.append(fac.createFieldAccess( /*fb type name*/var.getTypeName(), var.getFieldSelector(), Type.INT, Constants.GETFIELD));
				}
				else if (type == TypeConstant.T_REAL)
				{
					il.append(fac.createFieldAccess( /*fb type name*/var.getTypeName(), var.getFieldSelector(), Type.FLOAT, Constants.GETFIELD));
				}
				else if (type == TypeConstant.T_WSTRING)
				{
					il.append(fac.createFieldAccess( /*fb type name*/var.getTypeName(), var.getFieldSelector(), Type.STRING, Constants.GETFIELD));
				}
				else if (type == TypeConstant.T_DERIVED)
				{
					il.append(fac.createFieldAccess( /*fb type name*/var.getTypeName(), var.getFieldSelector(), new ObjectType(var.getFieldSelectorTypeName()), Constants.GETFIELD));
				}
				else
				{
	...
	InstructionList emitStoreVariable(IECSymbolicVariable var)
	{
		...
		else if (type == TypeConstant.T_DERIVED /*including function blocks*/)
		{
			...
			{    /* reference to a variable in the derived variable */
				...
				{
					il.append(InstructionConstants.SWAP);
					il.append(fac.createFieldAccess( /*fb type name*/var.getTypeName(), var.getFieldSelector(), Type.INT, Constants.PUTFIELD));
				}
				else if (type == TypeConstant.T_REAL)
				{
					il.append(InstructionConstants.SWAP);
					il.append(fac.createFieldAccess( /*fb type name*/var.getTypeName(), var.getFieldSelector(), Type.FLOAT, Constants.PUTFIELD));
				}
				else if (type == TypeConstant.T_WSTRING)
				{
					il.append(InstructionConstants.SWAP);
					il.append(fac.createFieldAccess( /*fb type name*/var.getTypeName(), var.getFieldSelector(), Type.STRING, Constants.PUTFIELD));
				}
				else if (type == TypeConstant.T_DERIVED)
				{
					il.append(InstructionConstants.SWAP);
					il.append(fac.createFieldAccess( /*fb type name*/var.getTypeName(), var.getFieldSelector(), new ObjectType(var.getFieldSelectorTypeName()), Constants.PUTFIELD));
				}
				else
				{
					error("IO variables in function blocks can " + "not have type " + type);
				}
			}
			else
			{    /* the derived variable itself */
				il.append(InstructionConstants.THIS);
				il.append(InstructionConstants.SWAP);
				il.append(fac.createFieldAccess(className, varName, new ObjectType(var.getTypeName()), Constants.PUTFIELD));
			}
		}
		else
		{
	...
	public void emitStackSpace(int size)
	{
		...
		{
			error("param size (emitStackSpace) must be 1 or 2." + " Probably you have used nonsupported IL constructs.");
		}
	}
	...
	private void emitLDN(Object arg)
	{
		...
		{
			System.err.println("LDN can only be used on the BOOL datatype." + "Not on " + arg);
	...
	private void emitSTN(Object arg)
	{
		if (arg instanceof IECConstant)
		{
			System.err.println("Fatal error: Operator STN cannot " + "store in constant");
		}
		else if (arg instanceof IECVariable)
		{
			 ...
			 */
			if ((var.getType() == TypeConstant.T_BOOL) || ((var instanceof IECSymbolicVariable) && (var.getType() == TypeConstant.T_DERIVED) && ((IECSymbolicVariable) var).getFieldSelectorType() == TypeConstant.T_BOOL))
			{
			...
			{
				error("STN cannot store i variable of type " + var.getType() + ". Only in BOOL variables.");
			}
		}
		else
		{
	...
	private void emitST(Object arg)
	{
		if (arg instanceof IECConstant)
		{
			System.err.println("Fatal error: Operator ST cannot " + "store in constant: " + arg);
		}
		else if (arg instanceof IECVariable)
		{
			IECVariable var = (IECVariable) arg;
			...
			{
				error("Store for " + var.getType() + " not yet implemented");
			}
		}
		else
		{
