...
public abstract class ProgramAndFBBuilder implements Builder {
    ...
    public void emitVarField(String varName, Object type,
        ...
        if (type instanceof TypeConstant) {
            ...
            emitVarField(varName,(TypeANY_ELEMENTARY)type,
                         global, inputOutputVar);
        }
        else if (type instanceof TypeFUNCTION_BLOCK) {
            emitVarField(varName,(TypeFUNCTION_BLOCK)type,
                         global, inputOutputVar);
        }
        else {
    ...
    public void emitVarField(String varName, TypeANY_ELEMENTARY type,
        ...
        if ( t == TypeConstant.T_BOOL ||
             t == TypeConstant.T_DINT ||
             t == TypeConstant.T_REAL ||
             t == TypeConstant.T_WSTRING) {
            /*create a field*/
    ...
    public void emitVarField(String varName, TypeFUNCTION_BLOCK type,
		...
		FieldGen var = new FieldGen (accessFlags,fbType,
									 varName,constPoolGen);
		classGen.addField(var.getField());
    ...
    InstructionList emitLoadVariable (IECSymbolicVariable var) {
		...
		if (type == TypeConstant.T_BOOL) {
                    ...
                    if (var.getFieldSelector() != null) {/* a variable in the derived variable*/
                        //first get reference to function block object
			il.append(InstructionConstants.THIS);
			il.append(fac.createFieldAccess(className, varName, new ObjectType(var.getTypeName()),
                                                        Constants.GETFIELD));
			//then load value
			...
			if (type == TypeConstant.T_BOOL) {
				il.append(fac.createFieldAccess(/*fb type name*/var.getTypeName(),
				...
				/*fb vars only of type bool*/
                            System.err.println("IO variables in function blocks can "+
                                               "only be of type BOOL, other types "+
                                               "should soon be implemented.");
                            errorsPresent = true;
			}
                    }
                    else { /*the derived variable itself*/
                        System.err.println("**********");
                        errorsPresent = true;
                    }
                }
    ...
    public void emitIL_SIMPLE_OPERATION(String operator, Object arg) {
        ...
        try {op = IlSimpleOperator.getOperator(operator);}
    ...
    InstructionList emitLoadG(Object arg){
		...
		if (arg instanceof IECConstant){
                    ...
                    if (t == TypeConstant.T_DINT) {
                        ...
                        System.err.println("Loading of "+ arg.toString()+ " not yet "+
                                           "implemented.");
                        errorsPresent = true;
                    }
                }
     ...
     */
    private void emitST(Object arg){//ej färdig
        if (arg instanceof IECConstant){
    ...
    private void emitAND(Object arg) {
        if (arg instanceof IECConstant){
            TypeConstant t = ((IECConstant)arg).getType();
            if (t == TypeConstant.T_BOOL) {
                if (!((TypeBOOL)arg).getValue()) { //result AND FALSE -> False
    ...
    private void emitANDN(Object arg) {
        if (arg instanceof IECConstant){
            TypeConstant t = ((IECConstant)arg).getType();
            if (t == TypeConstant.T_BOOL) {
                if (((TypeBOOL)arg).getValue()) { //result ANDN TRUE -> False
					ilRun.append(InstructionConstants.POP);
                    ilRun.append(InstructionConstants.ICONST_0);
                }
                //else  result
    ...
    private void emitOR(Object arg) {
        if (arg instanceof IECConstant){
            TypeConstant t = ((IECConstant)arg).getType();
            if (t == TypeConstant.T_BOOL) {
                if (((TypeBOOL)arg).getValue()) { //result OR TRUE -> TRUE
                    ilRun.append(InstructionConstants.POP);
    ...
    public void emitIL_EXPRESSION(String operator,TypeConstant t, Object arg) {
        ...
        try {op = IlExprOperator.getOperator(operator);}
        catch (IllegalOperatorException e) {
            System.err.println("Illegal operator: " + operator);
        }
        if (op == IlExprOperator.AND ) { //Logical AND
            emitExprAND(t,arg);}
        else if (op == IlExprOperator.OR) { //Logical OR
            emitExprOR(t,arg);}
        //  else if (op == IlExprOperator.XOR) { //Logical Exclusive OR
		else if (op == IlExprOperator.ANDN) { //Logical e1 AND NOT e2
			emitExprANDN(t,arg);}
        ...
        //          else if (op == IlExprOperator.NE) {
            ...
            System.err.println("Not implemented: " +operator+
                               ", operand of type: " + t);
        }
    }
    ...
    private void emitExprAND(TypeConstant t, Object arg){
        ...
        else {System.err.println("AND not implemented for type: " + t);}
    }
    ...
    private void emitExprOR(TypeConstant t, Object arg){
        ...
        else {System.err.println("OR not implemented for type: " + t);}
    }
	...
	*/
    private void emitExprORN(TypeConstant t, Object arg){
        ...
        else {System.err.println("ORN not implemented for type: " + t);}
    }
	...
	public void emitIL_FB_CALL_End(BranchInstruction callCondition) {
		if (callCondition != null /*call operator != CAL,
	...
	public void emitIL_FB_CALL_Run(String fbName, String fbTypeName) {
		...
		ilRun.append(fac.createFieldAccess(className,fbName,fbType,
										   Constants.GETFIELD));
		ilRun.append(fac.createInvoke(fbTypeName,"run",Type.VOID,
									  Type.NO_ARGS,
    ...
    public void emitIL_FB_CALL(IlCallOperator op,String fb_name,Object[] args){
		...
		if (fb_name.equals("print") || fb_name.equals("println")) {
			...
			System.err.println("Calls to function blocks not yet " +
							   "implemented");
		}
		/* set conditional target */
    ...
    private void emitPRINT(Object[] args, String print_type) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof TypeWSTRING) {
                ilRun.append(
                             fac.createFieldAccess("java.lang.System","out",
                                                   pStream,Constants.GETSTATIC));
                ilRun.append(new PUSH(constPoolGen,((TypeWSTRING)args[i]).getValue()));
                ilRun.append(fac.createInvoke("java.io.PrintStream",
				...
				IECVariable var = (IECVariable)args[i];
                                ilRun.append(fac.createFieldAccess("java.lang.System",
                                                                   "out",pStream,
                                                                   Constants.GETSTATIC));
                                ilRun.append(emitLoadVariable(var));
				if (var.getType() == TypeConstant.T_BOOL) {
                                    ...
                                    errorsPresent = true;
                                    System.err.println("Cannot print things of type: " +
                                                       var.getType());
				}
