...
import java.util.*;
public class JavaBytecodeGenerator implements CodeGenerator,SimpleNodeVisitor {
	...
	*/
    ...
    public JavaBytecodeGenerator(SimpleNode abstractSyntaxTreeRoot,
								 String outputDir/*not used*/){
...
//          }
    ...
    public Object visitIL_EXPRESSION(ASTil_expression n, Object o){
        ...
        if (children.length == 0){
            ...
            try{
                ASTsimple_instr_list simInstrList =
                    (ASTsimple_instr_list)children[0];
    ...
    public Object visitIL_JUMP_OPERATION(ASTil_jump_operation n, Object o) {
        ...
        Node[] children = n.getChildren();
        IlJumpOperator op =
			(IlJumpOperator)((SimpleNode)children[0]).visit(this,null);
        String label=
			(String)((SimpleNode)children[1]).visit(this,new String());
    ...
    public Object visitIL_JUMP_OPERATOR(ASTil_jump_operator n, Object o) {
        ...
        try {op = IlJumpOperator.getOperator(n.getName());}
    ...
    public Object visitNUMERIC_LITERAL(ASTnumeric_literal n, Object o){
		...
		if (p == null) {
			System.err.println("This constant type is not implemented: "+
							   n.getName() + ". Try typed constant "+
							   "(e.g. DINT#5)");
			//errorsPresent = true;
		}
        return p;
    ...
    public Object visitVARIABLE(ASTvariable n, Object o){
        ...
        if (n.isDirectVariable()) {
			...
			if (n.isFunctionBlock()){
				return new IECSymbolicVariable(n.getName(),
                                                               n.getType(),
                                                               n.getTypeName(),
                                                               n.getFieldSelector(),
                                                               n.getFieldSelectorType());
			}
			else {
				System.err.println("Function blocks are the only derived "+
								   "datatypes allowed");
				//errorsPresent = true;
				return null;
			}
	...
	public Object visitSIMPLE_TYPE_NAME(ASTsimple_type_name n, Object o) {
		...
		return null; /*if declaredDataType. Should be changed when
    ...
    public Object
		visitDERIVED_FUNCTION_BLOCK_NAME(ASTderived_function_block_name n,
										 Object o){
        System.out.println("   " + n.toString());
        return n.getName();
	}
	...
	public Object visitLOCATED_VAR_DECL(ASTlocated_var_decl n ,Object o) {
		...
		int base = 0;
		Node[] children = n.getChildren();
		if (children.length == 3) {//always 2 or 3, and in case of 3
			base = 1;           //we don't care about varname anyway
		}
		IECDirectVariable location =
			(IECDirectVariable)((SimpleNode)children[base]).visit(this,null);
		...
		if (type instanceof TypeConstant) {
			...
			if (!((TypeConstant)type == TypeConstant.T_BOOL)) {
				System.err.println("Direct variables only implemented "+
			...
			System.err.println("Direct variables only implemented "+
							   "for BOOL. Either the variable should "+
							   "just be declared BOOL or initialised with "+
							   "a boolean value");
				//errorsPresent = true;
		}
		return null;
	...
	public Object visitLOCATED_VAR_SPEC_INIT(ASTlocated_var_spec_init n,
		...
		if (children[0] instanceof ASTsimple_spec_init) {
			...
			System.err.println("Initialisation of located variables only "+
							   "implemented for simple BOOL");
	...
	public Object visitIO_VAR_DECLARATIONS(ASTio_var_declarations n,Object o) {
		System.out.println("   " + n.toString());
		SimpleNode child =
			(SimpleNode)(n.getChildren())[0]; //n always has exactly one child
		if (child instanceof ASTinput_output_declarations) {
			child.visit(this,null);
		}
		else { //child instanceof ASTinput_declaration
               //child instanceof ASToutput_declarations
			System.err.println("Only var_in_out is implemented, not var_in "+
							   "or var_out");
	...
	public Object visitVAR_DECLARATION(ASTvar_declaration n, Object o){
		...
		if (child instanceof ASTfb_name_decl) {
			System.err.println("ASTfb_name_decl found!!!! This is a wonder "+
							   "please keep your program and send it to the "+
							   "constructor");
		}
    ...
    public Object visitVAR1_INIT_DECL(ASTvar1_init_decl n, Object o) {
		...
		//else if (type instanceof DERIVED){
			...
			System.err.println("in var1_init_decl: this alternative not "+
							   "yet implemented. Probably derived type");
		}
        return null;
    }
    ...
    public Object visitVAR1_DECLARATION(ASTvar1_declaration n, Object o) {
		...
		//else if (type instanceof DERIVED){
			...
			System.err.println("in var1__declaration: this alternative not "+
							   "yet implemented. Probably derived type");
		}
        return null;
	}
    ...
    public Object visitSIMPLE_SPEC_INIT(ASTsimple_spec_init n, Object o) {
		...
		if (type instanceof TypeConstant) {
			...
			try {
				TypeANY init = (TypeANY)((SimpleNode)children[1]).
					visit(this,null);
				if (init.getType() == (TypeConstant)type) {
					return init;
				}
				else {
					System.err.println("Error: Nonconsistent types in : " +
									   type +" and "+ init.getType());
			...
			System.err.println("in simple_spec_init: this alternative "+
							   "not yet implemented");
		}
        return null;
    }
	...
	public Object visitIL_FB_CALL(ASTil_fb_call n, Object o) {
		System.out.println("   " + n.toString());
		Node[] children = n.getChildren();
		IlCallOperator op = (IlCallOperator)((SimpleNode)children[0]).
			visit(this,null); //get operator
		String fbName = (String)((SimpleNode)children[1]).visit(this,null);
		String fbTypeName = "Fbtest2";
		...
		if (children.length == 3 /* is there any arguments*/) {
			if (children[2] instanceof ASTil_operand_list) {
				...
				Node[] argNodes =
					(Node[])((SimpleNode)children[2]).visit(this,null);
				if (argNodes != null) {
			...
			else {/*children[2] instanceof ASTil_param_list*/
				BranchInstruction callCondition =
					builder.emitIL_FB_CALL_Start(op); //check callCondition
	...
	public Object visitMULTI_ELEMENT_VARIABLE(ASTmulti_element_variable n,
											  Object o) {
		System.out.println("   " + n.toString());
		return null;
	}
	...
	public Object visitSTRUCTURED_VARIABLE(ASTstructured_variable n,
										   Object o) {
		System.out.println("   " + n.toString());
		return null;
	}
