...
*/
	...
	************************************************/
	...
	private void paintPortVakt(Graphics g)
	{
		...
		g.fillRect(inPortVaktUpperX, inPortVaktUpperY, portWidth, inPortVaktDepth);

		//Paint the lower part of utPortVakt
		g.fillRect(utPortVaktUpperX, utPortVaktUpperY, portWidth, utPortVaktDepth);

		//Paint the Ports if signals equal to False
		if (!rController.getInSignal(0))
		{
			g.fillRect(inPortVaktUpperX, (inPortVaktUpperY - portLenght), portWidth, portLenght);
		}

		if (!rController.getInSignal(1))
		{
			g.fillRect(utPortVaktUpperX, (utPortVaktUpperY - portLenght), portWidth, portLenght);
		}
	}
	...
	private void paintMatLyft(Graphics g)
	{
		...
		if ((!rController.getInSignal(2)) && (level == 0))
		{
			...
			g.fillRect((centerX - radiusX), (lowerY - baseHeight), (2 * radiusX), baseHeight);
			g.fillRect((centerX - radiusTopX), (lowerY - baseHeight - topHeight), (2 * radiusTopX), topHeight);
		...
		else
		{
			if ((!rController.getInSignal(2)) || (NotOnTop))
			{
				...
				g.fillRect((centerX - radiusX), (lowerY - baseHeight + partLengthInt), (2 * radiusX), (-partLengthInt));
				g.fillRect((centerX - radiusTopX), (lowerY - baseHeight + partLengthInt - topHeight), (2 * radiusTopX), topHeight);
			...
			else
			{
				...
				g.fillRect((centerX - radiusX), (lowerY - maxHeightY), (2 * radiusX), maxHeightY);
				g.fillRect((centerX - radiusTopX), (lowerY - maxHeightY - topHeight), (2 * radiusTopX), topHeight);
			}
		}
	}
	...
	public void paintUrMatning(Graphics g)
	{
		...
		{
			g.fillRect(upperX, upperY, longLength, width);
		}
		else
		{
			g.fillRect(upperX, upperY, shortLength, width);
		}
	}
	...
	private void paintHiss(Graphics g)
	{
		...
		g.fillRect((centerX - radiusX), lowerY, (2 * radiusX), lengthVisible);

		//paint vertical and horisontal bars
		g.fillRect((centerX - radiusX), (lowerY - level), (2 * radiusX), level);
		g.fillRect((centerX - radiusTopX), (lowerY - level), (2 * radiusTopX), radiusX);
	}
	...
	public void paintUrVan1(Graphics g)
	{
		...
		{
			g.fillRect((ballX + length1), (ballY - width - space), length1, width);
			g.fillRect((ballX + length3), (ballY - space), length0, width);
		}
		else
		{
			g.fillRect((ballX + width), (ballY - width - space), length1, width);
			g.fillRect((ballX + length1), (ballY - space), length2, width);
		}
	}
	...
	public void paintUrVan2(Graphics g)
	{
		...
		{
			g.fillRect((upperX + (longLength - shortLength)), upperY, shortLength, width);
		}
	}
	...
	public void paintLyftVan1(Graphics g)
	{
		...
		{
			g.fillRect(upperX, (lowerY - longLength), width, longLength);
		}
		else
		{
			g.fillRect(upperX, (lowerY - shortLength), width, shortLength);
		}
	}
	...
	private void paintArm(Graphics g)
	{
		...
		g.fillRect((xCenterPos - radiusX), (lowerY - level), (2 * radiusX), (level + 6));

		//Paint the horisontal bar and the PROPP
		if (angel >= xDiff1)
		{
			g.fillRect(xCenterPos, (lowerY - level), (angel - xDiff1 + proppSize / 2), radiusX);
			g.fillOval((xCenterPos + angel - xDiff1), (lowerY - level + proppDepth), proppSize, proppHeight);
		}
		else
		{
			g.fillRect((xLeftPos + angel + proppSize / 2), (lowerY - level), (xDiff1 - angel - proppSize / 2), radiusX);
			g.fillOval((xLeftPos + angel), (lowerY - level + proppDepth), proppSize, proppHeight);
		}
	}
}
