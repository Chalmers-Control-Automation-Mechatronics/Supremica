	...
	implements Runnable
{
		...
		throws Exception
	{
		...
		if (armLevel == armVertVan1Length)
		{
			...
			if (inSignals[15] && (armAngle == armHoriLength) && inSignals[13] &&!inSignals[14])
			{
			...
			else if (inSignals[15] && inSignals[13] &&!inSignals[14] && (armAngle < armHoriLength))
			{
				armAngle = armAngle + 2;
			}
			else if (armAngle > 0)
		...
		else if (armLevel == armVertLength)
		{
			...
			else if (inSignals[15] && inSignals[14] && (armAngle < armHoriLength))
			{
				armAngle = armAngle + 2;
			}
			else if (armAngle > 0)
