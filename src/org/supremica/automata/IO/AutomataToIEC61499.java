...

{
	...
	public void printSources(File directory)
	{	
		...
		try
		{
			...
			for (Iterator autIt = theProject.iterator(); autIt.hasNext();)
			{
				...
				File tmpFile = new File(directory.getParent() + "/AUTOGEN_" + tempProject.getAutomatonAt(0).getName() + ".fbt");
			...
			// Make the file
			File tmpFile = new File(directory.getParent() + "/AUTOGEN_SYNC.fbt");
			PrintWriter pw = new PrintWriter(new FileWriter(tmpFile));    
