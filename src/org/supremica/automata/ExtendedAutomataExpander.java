...
public class ExtendedAutomataExpander
{
    ...
    static class GuardAndActionPair {
		...
		public GuardAndActionPair(final String guard, final String actions) {
			super();
			this.guard = guard;
	...
	public static void expandTransitions(final ModuleSubject module)
	{
        ...
        for (final Proxy proxy : module.getComponentList()) {
			...
			final String curComponentName = curComponent.getName();
			...
			for (final EdgeSubject curEdge : edges)
			{
				final NodeProxy source = curEdge.getSource();
				final NodeProxy target = curEdge.getTarget();

				final LabelBlockProxy curLabel = curEdge.getLabelBlock();

				final GuardActionBlockProxy curBlock = curEdge.getGuardActionBlock();

				// do expansion only if guard and actions block is present
				if (curBlock != null)
				{
					final Set<GuardAndActionPair> expandedGuardsAndActions = createExpandedGuardAndActions(moduleVariables, curBlock);
					for (final GuardAndActionPair guardAndActions : expandedGuardsAndActions) {
	...
	private static Set<GuardAndActionPair> createExpandedGuardAndActions(final Map<String,VariableComponentProxy> moduleVariables, final GuardActionBlockProxy guardActionBlock) {
		...
		if (guardActionBlock.getGuards().size() > 0) {
			...
			final Script guardScript = new GroovyShell().parse(groovyfy(guardText, true));
	...
	private static Set<GuardAndActionPair> createExpandedGuardAndActionsGivenGuardBinding(final Map<String,VariableComponentProxy> moduleVariables, final Set<String> guardIdentifiers, final Binding guardBinding, final GuardActionBlockProxy guardActionBlock) {
		...
		if (guardActionBlock.getActions().size() > 0) {
			...
			final Map<String,Script> actionScripts = new LinkedHashMap<String,Script>();
			final List<BinaryExpressionProxy> actions = guardActionBlock.getActions();
			final GroovyShell shell = new GroovyShell();
			int i = 0;
			for (final String assignedIdentifier : actionsAssignmentIdents) {
			...
			// currently only integer variables
				...
				for (final List<Integer> values : combinationsOfAllActionExpressionVariableValues) {
					...
					for (final String identity : ((Map<String, ? extends Object>) guardBinding.getVariables()).keySet()) {
					...
					if (variableValuesWithinBounds(moduleVariables, actionResult)) {
						final String actionString = createActionsString(actionResult);
						final String guardString = constructGuardFromBinding(guardActionBinding);
				...
				if (variableValuesWithinBounds(moduleVariables, actionResult)) {
					final String actionString = createActionsString(actionResult);
					final String guardString = constructGuardFromBinding(guardBinding);
					guardsAndActionsFromExpansion.add(new GuardAndActionPair(guardString, actionString));
				}
			}
		} else { // No actions
	...
	private static String groovyfy(final String text, final boolean isGuard) {
	...
	private static Map<String, Integer> evaluateActions(
			final Map<String, Script> actionScripts,
			final Binding guardActionBindingOriginal) {
		final Map<String,Integer> actionResult = new LinkedHashMap<String, Integer>();
		for(final Map.Entry<String,Script> identifierAssignmentScriptPair : actionScripts.entrySet()) {
			final Binding guardActionBindingToBeModified = new Binding(new LinkedHashMap<Object,Object>(guardActionBindingOriginal.getVariables()));
			...
			final Object value = guardActionBindingToBeModified.getVariable(identifierAssignmentScriptPair.getKey());
	...
	private static boolean variableValuesWithinBounds(final Map<String, VariableComponentProxy> moduleVariables, final Map<String, Integer> actionsResult) {
		for (final String identifier : actionsResult.keySet()) {
	...
	private static String createActionsString(final Map<String, Integer> actionResult) {
		// make new actions
		String actions = "";
		for (final String curIdent : actionResult.keySet()) {
	...
	@SuppressWarnings("unchecked")
	private static Set<String> findExpressionIdentifiers(final Goal syntaxTree) {
		final Set<String> identifiers = new LinkedHashSet<String>();
		if (syntaxTree != null)
		{
			final Finder finder = new Finder(syntaxTree);
			final Set<Identifier> expressionIdents = finder.getExpressionIdentifiers();
			for (final Identifier curIdent : expressionIdents) identifiers.add(curIdent.a);
		}
		return identifiers;
	}

	@SuppressWarnings("unchecked")
	private static Set<String> findAssignmentIdentifiers(final Goal syntaxTree) {
		final Set<String> identifiers = new LinkedHashSet<String>();
		if (syntaxTree != null)
		{
			final Finder finder = new Finder(syntaxTree);
			final Set<Identifier> expressionIdents = finder.getAssignmentIdentifiers();
			for (final Identifier curIdent : expressionIdents) identifiers.add(curIdent.a);
		}
		return identifiers;
	}

	private static String constructActionText(final GuardActionBlockProxy curBlock) {
		String actionsText = "";
		for (final BinaryExpressionProxy curAction : curBlock.getActions()) actionsText = actionsText + curAction.toString() + "; ";
		return actionsText;
	}

	private static String constructGuardText(final GuardActionBlockProxy curBlock) {
		 return "(" + join(curBlock.getGuards(), ") & (") + ")";
	}

	private static Goal parse(final String text) {
    ...
    public static void dontExpandTransitions(final ModuleSubject module)
    {
        ...
        for (final Proxy proxy : module.getComponentList())
        {
            ...
            for (final Iterator<EdgeSubject> edgeIter = edges.iterator(); edgeIter.hasNext();)
            {
                final EdgeSubject curEdge = edgeIter.next();
                ...
                if (curBlock != null)
                {
                    ...
                    // get guards
                    final List<SimpleExpressionProxy> curGuards = curBlock.getGuards();
                    String guardText = "(";
                    if (curGuards.size() > 0 )
                    {
                        log.debug("ExtendedAutomataExpander.expandTransitions(): Guard");
                        for (final Iterator<SimpleExpressionProxy> iter = curGuards.iterator(); iter.hasNext();)
                        {
                    ...
                    // get actions
                    final List <BinaryExpressionProxy> curActions = curBlock.getActions();
                    String actionsText = "";
                    String origActionsText = "";
                    if (curActions.size() > 0 )
                    {
                        log.debug("ExtendedAutomataExpander.expandTransitions(): Actions");
                        for (final Iterator<BinaryExpressionProxy> iter = curActions.iterator(); iter.hasNext();)
                        {
                            final BinaryExpressionProxy curAction = iter.next();
	...
	private static EdgeSubject makeTransition(final NodeProxy from, final NodeProxy to, final LabelBlockProxy labelBlock, final String guardIn, final String actionIn)
	{
		...
		final EdgeSubject newEdge = factory.createEdgeProxy(from, to, labelBlock, guardActionBlock, null, null, null);

		return newEdge;
	}

	protected static <T> String join(final Collection<T> collection, final String delimiter) {
