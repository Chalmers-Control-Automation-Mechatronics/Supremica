...
public class BDDPartitionTypeFactory {
     ...
     */
    public static BDDPartitionAlgoWorker getPartitioningAlgorithmWorker(final BDDExtendedAutomata bddExAutomata, final SynthesisAlgorithm synType) {
        ...
        if (synType == SynthesisAlgorithm.PARTITIONBDD) {
            if (!bddExAutomata.orgExAutomata.getVars().isEmpty()) {
                ...
                final BDDPartitionSet eventPartitions = new BDDPartitionSetEve(bddExAutomata);
                final BDDPartitionCoordinator eventCoordinator = new BDDPartitionCoordinatorEve(eventPartitions);
                ...
                final BDDPartitionSet automatonPartitions = new BDDPartitionSetAut(bddExAutomata);
                final BDDPartitionCoordinator automatonCoordinator = new BDDPartitionCoordinatorAut(automatonPartitions);
            ...
            logger.info("RAS models: the event-based partitioning algorithm is choosen.");
            final BDDPartitionSet eventPartitions = new BDDPartitionSetEve(bddExAutomata);
            final BDDPartitionCoordinator eventCoordinator = new BDDPartitionCoordinatorEve(eventPartitions);
            parAlgoWorker = new BDDPartitionAlgoWorkerRan(eventPartitions, eventCoordinator);
        }
