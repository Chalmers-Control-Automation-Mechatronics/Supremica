...
public final class BDDPartitionCoordinatorAut extends BDDPartitionCoordinator{
     ...
     */
    private int[] selectedAutIndices;
    ...
    public BDDPartitionCoordinatorAut(BDDPartitionSet autPartitions) {
        super(autPartitions);
    ...
    public int pickOne(boolean forForward) {
        ...
        int selectedAutSize = getMostFollowers();
    ...
    private int getMostFollowers() {
        ...
        for (int automatonIndex = 0; automatonIndex < size;  automatonIndex++) {
            if (workset[automatonIndex]) {
                int depSize = dependentMatrix[automatonIndex][0];
                if (best < depSize) {
                    best = depSize;
                    selectedAutSize = 0;
                }
    ...
    private void record (boolean changed) {
        if (changed) {
            int depSize = dependentMatrix[choice][0];
            for (int i = 1; i <= depSize; i++) {
                int autIndex = dependentMatrix[choice][i];
                if (!workset[autIndex]) {
