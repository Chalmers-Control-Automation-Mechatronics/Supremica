...
public final class BDDPartitionCoordinatorEve extends BDDPartitionCoordinator{
     ...
     */
    private TIntHashSet candidateEventIndexSet;
    
    private TIntHashSet activeEventIndexSet;
     ...
     */
    private BDDPartitionHeuristicsLearner learner;
    ...
    @Override
    public int pickOne(boolean forForward) {
    ...
    private void buildFirstCandidates(boolean forForward) {
    ...
    private void buildSubsequentCandidates(boolean forForward) {       
        TIntObjectHashMap<TIntHashSet> eventIndex2DepEventIndexSetMap;
        if (forForward) {
            eventIndex2DepEventIndexSetMap = partitions.getForwardDependentComponentMap();
        } else {
    ...
    private void selectEventIndiceWithLargestDepSet(TIntHashSet compIndexSet,
                                                    TIntObjectHashMap<TIntHashSet> eventIndex2DepEventIndexSetMap) {
        int maxDepSize = -1;
    ...
    @Override
    public void advance(boolean forward, boolean changed) {
    ...
    private void record(boolean forward, boolean changed) {      
        if (changed) {
            if (firstTime) {
                firstTime = false;
            ...
            for (TIntIterator eventItr = eventIndex2DepEventIndexSetMap.get(choice).iterator(); eventItr.hasNext();) {
