...
public class BDDAutomaton
{
	...
	public BDDAutomaton(BDDManager manager, Automaton theAutomaton, BDDDomain sourceStateDomain, BDDDomain destStateDomain)
	{
		...
		transitionForwardDisjunctiveBDD = manager.zero();
		transitionBackwardDisjunctiveBDD = manager.zero();
	}
	...
	public void initialize()
	{
		...
		for (State currState : theAutomaton)
		{
			...
			{
				// Add all states that could be reach from the intial state by only unobservable events
				// include the initial state as well
				for (State epsilonState : currState.epsilonClosure(true))
				{
					int epsilonStateIndex = manager.getStateIndex(theAutomaton, epsilonState);
					manager.addState(initialStates, epsilonStateIndex, sourceStateDomain);
				}
			}
			if (currState.isAccepting())
			{
				manager.addState(markedStates, stateIndex, sourceStateDomain);
				// I guess it is possible to add all states that reach a marked state
				// with epsilon events to the set of marked states as well.
				// Is this done previously?
			}
			if (currState.isForbidden())
			{
				manager.addState(forbiddenStates, stateIndex, sourceStateDomain);
				// I guess it is possible to add all states that reach a forbidden state
				// with epsilon events to the set of forbidden states as well
				// Is this done previously?
