...
public class AutomataTransitionProjection {
    ...
    public ExtendedAutomaton projectEFA(final String name){
        ...
        final boolean result = checkForActionConsistency(exAutomatonIndex);
        if(!result)
            return null;
     ...
     */
    private boolean checkForActionConsistency(final int automatonIndex){
        final int[][][] stateEventTable = indexAutomata.getActionStateEventTable()[automatonIndex];
        final TIntHashSet alphabet = ExtendedAutomataIndexFormHelper.getTrueIndexes(indexAutomata.getAlphabetEventsTable()[automatonIndex]);
        for(final int ev : alphabet.toArray()){
            final HashSet<String> set = new HashSet<String>();
            for(final int[][] state : stateEventTable){
                final int[] actions = state[ev];
