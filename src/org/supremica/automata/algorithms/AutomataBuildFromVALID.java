...
public class AutomataBuildFromVALID
{
	...
	private static void automataFromVPRJ(Element root)
		throws Exception
	{
		Document subDoc = docBuilder.build(new File(filePath + File.separator + root.getChild("uses").
													getAttributeValue("module") + ".vmod"));
		...
		throws Exception
	{
		...
		while (i.hasNext())
		{
			...
			Document subDoc = docBuilder.build(new File(filePath + File.separator +
														element.getAttributeValue("graph") + ".dgrf"));
			Element subRoot = subDoc.getRootElement();
		...
		{
			element = (Element) i.next();
			subDoc = docBuilder.build(new File(filePath + File.separator + element.getChild("instance").
											   getAttributeValue("module") + ".vmod"));
			...
			{
				modificationList = (List) definitionHash.get(element.getAttributeValue("range").
															 substring(1));
			}
			else
			{
			...
			st = new StringTokenizer(element.getChild("instance").
												 getAttributeValue("name"), "[$]");
			oldAutomatonName = st.nextToken();
			if (!dummyVariable.equals(st.nextToken()))
				throw new Exception("Something is wrong with the dummy variables in " +
				...
				while (k.hasNext())
				{
					...
					if (st.hasMoreTokens())
					{
						...
						while (true)
						{
							...
							if (alias.getAttributeValue("new").
								equals(nameParamElement.getAttributeValue("name")))
							{
								oldEventName = alias.getAttributeValue("old");
								break;
							}
							if (!l.hasNext())
								throw new Exception("Something is wrong with the aliases in " +
													subRoot.getAttributeValue("name") + ".vmod.");
						}
		...
		throws Exception
	{
		...
		while (i.hasNext())
		{
			...
			if (element.getAttributeValue("isLoop").equals("1"))
				destState = currAutomaton.getStateWithId(element.getChild("source").
														 getAttributeValue("name"));
			else
				destState = currAutomaton.getStateWithId(element.getChild("target").
														 getAttributeValue("name"));
