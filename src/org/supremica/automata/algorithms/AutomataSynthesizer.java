    ...
    implements Stoppable
{
	...
	else if (synthesizerOptions.getSynthesisAlgorithm() == SynthesisAlgorithm.Modular)
	{
		...
		min.setName(null);
	        ...
	        /* What is this doing here?!! The indentation is all wrong too! 
		   Get in the game!  :o) /hguo
	...
	*/
    ...
    /**
     * Standard method for synthesizing a controllable and nonblocking supervisor.
     */
    public static Supervisor synthesizeControllableNonblocking(Automata automata)
    {
	try
	{
	    // Get options
	    SynchronizationOptions syncOptions = SynchronizationOptions.getDefaultSynthesisOptions();
	    SynthesizerOptions synthOptions = SynthesizerOptions.getMonolithicCNBSynthesizerOptions();
	    
	    // Synthesize!
	    AutomataSynthesizer synthesizer = new AutomataSynthesizer(automata, syncOptions, synthOptions);
	    // It's actually monolithic, but what's the difference.
	    return new ModularSupervisor(synthesizer.execute()); 
	}
	catch (Exception ex)
	{
	    logger.error("Error when synthesizing supervisor. ", ex);
	    return null;
	}
    }
}
