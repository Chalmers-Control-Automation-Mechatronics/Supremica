	...
	implements AutomataSerializer
{
		...
		throws Exception
	{
		...
		if (pos != -1)
		{
			autname = autname.substring(0, pos);
		}
		...
		throws Exception
	{
		...
		if (first < 1)
		{
			throw new Exception("bad allocation event (type): " + event);    // -1 (not found) or 0 (no type desc)
		}
		...
		if (type.equals("Book"))
		{
			btype = true;     // <-- book event
		}
		else if (type.equals("Unbook"))
		{
			btype = false;    // <-- unbook event
		}
		else
		{
			throw new Exception("bad allocation event (type=" + type + ") : " + event);
		}
		...
		if (second <= first)
		{
			throw new Exception("bad allocation event (res): " + event);
		}
		...
		if (third <= second)
		{
			throw new Exception("bad allocation event (owner): " + event);
		}
