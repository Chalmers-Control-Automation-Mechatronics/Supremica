...
import org.supremica.util.BDD.IncompleteStateTree.StateTreeNode;
...
public class AutomataBDDSynthesizer
{
	...
	private boolean doNonBlocking, doControllable;

	public AutomataBDDSynthesizer(org.supremica.automata.Automata theAutomata, boolean supNB, boolean supC)
		throws Exception
	{
		this.doControllable = supC;
		this.doNonBlocking = supNB;
                
		try
		{
			Builder bu = new Builder(theAutomata);

			bddAutomata = bu.getBDDAutomata();

			//sup = new Supervisor(ba, ba.getAutomataVector());
			supervisor = SupervisorFactory.createSupervisor(bddAutomata, bddAutomata.getAutomataVector());
		} catch (Exception e) {
			cleanup();
			throw e;
		}
	...
	/**
	 * C++ style destructor.
	 * <b>This function MUST be called before creating any new AutomataBDDVerifier objects</b>
	 *
	 */
	public void cleanup()
	{
		if (getSupervisor() != null) getSupervisor().cleanup();
		if (bddAutomata != null) bddAutomata.cleanup();
	}
                
    public int UCStates()
    {
        return getSupervisor().getUncontrollableStates();
    }
    
    public int coReachStates()
    {
        return getSupervisor().getCoReachables();
    }
    
    public int reachStates()
    {
        return getSupervisor().getReachables();
    }
    
    public int deadStates()
    {
        return getSupervisor().getDeadlocks();
    }
	 ...
	 *
	 */
	public int computeSafeStates()
	{
            return getSupervisor().getSafeStates(doNonBlocking, doControllable);
	}
	...
	public OnlineBDDSupervisor extractOnlineSupervisor()
	{
		...
		try	{
			int safe_states = computeSafeStates();       
			ret = new OnlineBDDSupervisor(bddAutomata, safe_states);
		} finally {
			// we dont own these objects anymore
			getSupervisor().cleanup();
			supervisor = null;
			bddAutomata = null;
		}
		return ret;
	...
	public void generateCode()
	{
		...
		if (returnVal == JFileChooser.APPROVE_OPTION)
		{
			try
			{
				...
				int safe = computeSafeStates();
				Vector<DisablingPoint> all_bad = getSupervisor().getUnsafeTransitionTree(safe);

				bddAutomata.deref(safe);
	...
	private void extract_tree_rec(IncompleteStateTree.StateTreeNode nod, PrintStream ps, boolean f)
	{
		...
		Vector<StateTreeNode> children = nod.getChildren();
		...
		for (IncompleteStateTree.StateTreeNode next : children)
		{
			if (!first)
			{
				ps.print(" OR ");
