...
{
    protected static Logger logger = LogManager.getLogger(ComputerHumanExtender.class);
    ...
    public ComputerHumanExtender(final Automata theAutomata, final int k, final int m)
    {
        constructor(theAutomata, k, m);
    }

    public ComputerHumanExtender(final Alphabet theAlphabet, final int k, final int m)
    {
        constructor(theAlphabet, k, m);
    }

    private void constructor(final Automata theAutomata, final int k, final int m)
    {
        if (k < 1)
        {
        ...
        try
        {
            final Alphabet inputAlphabet = AlphabetHelpers.getUnionAlphabet(theAutomata, true, true);
    ...
    private void constructor(final Alphabet theAlphabet, final int k, final int m)
    {
    ...
    protected void buildAlphabets(final Alphabet theAlphabet)
    {
        ...
        for (final Iterator<LabeledEvent> evIt = theAlphabet.iterator(); evIt.hasNext(); )
        {
            ...
            final LabeledEvent newEvent = currEvent.clone(); // was: new LabeledEvent(currEvent); not polite!
...
//		LabeledEvent passEvent = new LabeledEvent("pass");
        ...
        final State initialState = newAutomaton.createUniqueState("qe_0_1");
 ...
 */
        ...
        final State[] higherMStates = new State[k+1];
        ...
        for (int im = m; im >= 1; im--)
        {
            ...
            for (int ik = k; ik >= 0; ik--)
            {
                ...
                if (!(ik == 0 && im == 1))
                {
                    ...
                    newState = newAutomaton.createUniqueState("qe_" + ik + "_" + im);
 ...
 */
                ...
                if (ik < k)
                {
                    ...
                    for (final Iterator<LabeledEvent> evIt = operatorIncreaseEvents.iterator();
                    evIt.hasNext(); )
                    {

                        // System.err.println("added operator arc");
                        final LabeledEvent currEvent = evIt.next();
                        final Arc newArc = new Arc(newState, prevUpperState, currEvent);
                    ...
                    if (ik == k-1)
                    {
                        ...
                        final Arc newArc = new Arc(prevUpperState, newState, passEvent);
                ...
                if (im < m)
                {
                    final State higherMState = higherMStates[ik];

                    // Add all operator reset events to the corresponding level in m+1
                    for (final Iterator<LabeledEvent> evIt = operatorResetEvents.iterator();
                    evIt.hasNext(); )
                    {
                        // System.err.println("added operator arc");
                        final LabeledEvent currEvent = evIt.next();
                        final Arc newArc = new Arc(newState, higherMState, currEvent);
                ...
                else if (im == m)
                {
                    ...
                    {
                        final LabeledEvent currEvent = evIt.next();
                        final Arc newArc = new Arc(newState, initialState, currEvent);
