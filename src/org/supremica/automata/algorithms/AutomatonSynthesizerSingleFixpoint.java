...
{
    public AutomatonSynthesizerSingleFixpoint(final Automaton theAutomaton, final SynthesizerOptions synthesizerOptions)
    {
    ...
    // ---------------------------------------------------
	private void remove(final int mask)
    {
        final int inv_bit = ~mask;
        final Iterator<State> stateIt = theAutomaton.stateIterator();
	...
	private void add(final int mask)
    {
        final Iterator<State> stateIt = theAutomaton.stateIterator();
        while (stateIt.hasNext())
        {
	...
	private long union(final int mask, final int set, final int remove)
    {
        final int inv_remove = ~remove;
        long count = 0;
        final Iterator<State> stateIt = theAutomaton.stateIterator();
        while (stateIt.hasNext())
        {
    ...
    private long intersect(final int mask, final int set, final int remove)
    {
        long count = 0;
        final int inv_remove = ~remove;
        final Iterator<State> stateIt = theAutomaton.stateIterator();
    ...
    private void invert(final int mask)
    {
        final int inv_mask = ~mask;
        final Iterator<State> stateIt = theAutomaton.stateIterator();
        while (stateIt.hasNext())
        {
            final State currState = stateIt.next();
    ...
    private long cardinality(final int mask)
    {
        long count = 0;
        final Iterator<State> stateIt = theAutomaton.stateIterator();
        while (stateIt.hasNext())
    ...
    // ---------------------------------------------------
    private long forbiddIfNot(final int mask)
    {
        long count = 0;
        final Iterator<State> stateIt = theAutomaton.stateIterator();
        while (stateIt.hasNext())
    ...
    private long forbiddIf(final int mask)
    {
        long count = 0;
        final Iterator<State> stateIt = theAutomaton.stateIterator();
        while (stateIt.hasNext())
    ...
    // B1 : doB(1, 2)
	private void doB(final int mask, final int set)
    {
        final Iterator<State> stateIt = theAutomaton.stateIterator();
        while (stateIt.hasNext())
        {
            ...
            if ((currState.sethelper & mask) != 0)
            {
                final Iterator<Arc> arcIt = currState.incomingArcsIterator();
                while (arcIt.hasNext())
                {
    ...
    private void doB_inverse(final int mask, final int set)
    {
        final Iterator<State> stateIt = theAutomaton.stateIterator();
        while (stateIt.hasNext())
        {
    ...
    // A1: doA(3,4)
    private void doA(final int mask, final int set)
    {
        final Iterator<State> stateIt = theAutomaton.stateIterator();
        while (stateIt.hasNext())
        {
    ...
    /** see doA() */
    private boolean stayInside(final State s, final int mask)
    {
        final Iterator<Arc> arcIt = s.outgoingArcsIterator();
        while (arcIt.hasNext())
        {
            final Arc currArc = arcIt.next();
            final State s2 = currArc.getToState();
    ...
    private boolean stayInsideUncontrollable(final State s, final int mask)
    {
        final Iterator<Arc> arcIt = s.outgoingArcsIterator();
        while (arcIt.hasNext())
        {
