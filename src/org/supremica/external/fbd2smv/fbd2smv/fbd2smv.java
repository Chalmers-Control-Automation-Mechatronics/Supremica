...
public class fbd2smv
{
    ...
    public fbd2smv(String fbdProjectPath, String smvBlocksPath, String smvOutputPath) throws IOException
    {
		...
		for (int i=0; i<programs.size(); i++)
		{
			...
			LinkedList boxesList = new LinkedList();


			translateBoxNames(boxesList);
    ...
    void printMainModule(PrintWriter pw, LinkedList programs, LinkedList varBooleans, LinkedList varIntegers)
    {
		...
		*/
		...
		pw.println("");
		// Loop med utskrift av block här
		...
		for (int i=0; i<inputElementIndices.size(); i++)
			{
				...
				String sourceIndex = S.valueOf(((Tuple3)inputElementIndices.get(i)).x);
				int    sourceOutputNumber = ((Tuple3)inputElementIndices.get(i)).y;
				...
				if (theFBDElement.getType().equals("variable") ) 
				...
				else if (theFBDElement.getType().equals("box") ) 
					{
					    ...
					    System.out.println("BLOCKNAME (input): " + ((BOX)theFBDElement.getElement()).getName() + "_" + ((BOX)theFBDElement.getElement()).getIndex());
					    ...
					    if ((box2.getName().equals("{\\div}")))
						{
						    ...
						    while (box2.getName().equals("{\\div}"))
							{
							    System.out.println("box2.index=" + box2.getIndex());
							    divInputIndices = inputElementIndices(box2.getIndex(), arcs);
							    System.out.println("divInputIndices.get(0)=" + ((Tuple3)divInputIndices.get(0)).x);
							    box2 = (BOX)boxes.get(divInputIndices.get(0));
							    System.out.println("box2_new.index=" + box2.getIndex());


							}
						    sourceIndex = S.valueOf(((Tuple3)divInputIndices.get(0)).x);
						    sourceOutputNumber = ((Tuple3)divInputIndices.get(0)).y;
						    theFBDElement = getElementByIndex(program, sourceIndex);
						}
					    ...
					    Block blkTmp = (Block)smvBlocks.get(((BOX)theFBDElement.getElement()).getName());

					    System.out.println("Block is null: " + (blkTmp==null));
		...
		for (int i=0; i<outputElementIndices.size(); i++)
			{
				...
				String targetIndex = S.valueOf(((Tuple3)outputElementIndices.get(i)).x);
				int targetInputNumber = ((Tuple3)outputElementIndices.get(i)).y;
		...
		for (Iterator arcIt = theArcs.iterator(); arcIt.hasNext(); )
			{
				...
				if (boxIndex.equals(S.valueOf(currARC.getTargetIndex()))) 
					{
						theInputElementIndices.add(new Tuple3(currARC.getSourceIndex(), currARC.getSourceOutputNumber(), currARC.getTargetInputNumber()));
					}
            }
		...
		for (Iterator arcIt = theArcs.iterator(); arcIt.hasNext(); )
			{
				...
				if (boxIndex.equals(S.valueOf(currARC.getSourceIndex()))) 
					{
						theOutputElementIndices.add(new Tuple3(currARC.getTargetIndex(), currARC.getTargetInputNumber(), currARC.getSourceOutputNumber()));
					}
            }
    ...
    class Tuple3 implements java.lang.Comparable
    {
		public int x;
		public int y;
		public int z;
