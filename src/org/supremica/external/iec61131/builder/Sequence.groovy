...
class Transition {
	...
	Identifier name
	List from = []	//Step identifiers
	List to = []	//Step identifiers
	Expression guard
	Sequence sequence
	static final ENABLED_NAME = new Identifier('enabled')
	...
	List getRuntimeAssignments(Scope parent) {
		...
		String mainExpr = [*from.collect{"${it + 'X'}"}, "($guard)"].join(' and ')
...
class Step {
	...
	Sequence sequence
	boolean marked
	List variables = [new InternalVariable(name:new Identifier('X')),
	                  new InternalVariable(name:new Identifier('activation')),
	                  new InternalVariable(name:new Identifier('deactivation'))]
	List statements = []		
	...
	List getRuntimeAssignments(Scope parent) {
		...
		variables.find{it.name == new Identifier('X')}.markedValue = ([*sequence.steps.findAll{it.marked}, sequence.steps[0]][0] == this)
		Scope scope = [self:this, parent:parent]
		ControlCodeBuilder ccb = new ControlCodeBuilder()
		Closure buildExpr = {stepEntrance ->
			sequence.transitions.findAll{(stepEntrance ? it.to : it.from).any{it == name}}.collect{t -> sequence.name + t.name + Transition.ENABLED.name}.text.join(' or ')
		}
		List execStatements = []
