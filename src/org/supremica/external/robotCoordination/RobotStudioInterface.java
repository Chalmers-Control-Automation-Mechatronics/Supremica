...
public class RobotStudioInterface
{
	...
	// Initialize jacoZoom
	static {com.inzoom.comjni.Dll.runRoyaltyFree(643622874);}
		...
		implements RobotCell, DAppEvents
	{
		...
		public RSRobotCell(String stationName)
		{
			try
				...
				for (int i=0; i<getMechanismsWithRole(RsKinematicRole.rsKinematicRoleRobot).size(); i++)
			...
			throws Exception
		{
			...
			for (Iterator mechanismIt = list.iterator(); mechanismIt.hasNext();)
			{
		...
		public void quit()
		{
			...
			logger.fatal("RobotStudio is shutting down. Please tell Hugo " +
						 "if, when and how you got this message!");
			// clean();
		}
		...
		private class MechanismListener extends _MechanismEventsAdapter
		{
			...
			public MechanismListener(Mechanism mechanism, Path path)
			{
				...
				try
				{
					...
					{
						logger.debug(mechanism.getName() + " is already inside the zone " +
									((Collider) it.next()).getName() + " at target " +
									path.getTargetRefs().item(var(1)).getName() + ".");
					}
				}
				catch (Exception e)
				{
			...
			public synchronized int collisionStart(RsObject collidingObject)
			{
				try
				{
					...
					*/
					...
					if (indexZone<=0)
					{
						logger.warn("It appears that " + mechanism.getName() +
									" has collided with '" + objectName +"'.");
						return 0;
		...
		implements Robot
	{
		 ...
		 */
		private class ControllerListener extends _MechanismEventsAdapter
		{
		 ...
		 */
		private class SpanGenerator extends SimulationListener
		{
