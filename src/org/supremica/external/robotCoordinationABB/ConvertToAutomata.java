...
public class ConvertToAutomata
{
	...
	*/
	...
	public static void conversionToAutomata(File stationFile)
	{
	    ...
	    robotAutomata.clear();
	    int nbrOfRobot = 0;
	    for (Iterator listIt = stationName.getChildren().iterator(); listIt.hasNext();)
	    {
	...
	private static void convertMutexZonesIntoAutomata(Element elemento)
	{
		...
		{
			for (Iterator listIt = elem.getChildren().iterator(); listIt.hasNext();)
			{
		...
		else logger.info("MutexZones is an empty set");
	}
	...
	private static void convertRobotIntoAutomaton(Element el)
	{
	    ...
	    for (Iterator listIt = allowedMotions.getChildren().iterator(); listIt.hasNext();)
	    {
	...
	private static Element searchPath(Element rob, String s)
		{
			...
			{
				for(Iterator listIt = rob.getChildren().iterator(); listIt.hasNext();)
				{
