...
public class DOPrelation
	 ...
	 * @param efa
	 */
	protected static void relation(final Relation relation,final EFA efa){
     ...
     * @param r is relation with type sequence
	 * @param from is start state for sequence
	 * @param to is end state for sequence
	 */
	protected static void sequence(final Relation r,
		...
		Object o = null;
		final List<Activity> myActivityList = new LinkedList<Activity>();

		myLastState = from;

		final List<Object> activityList = r.getActivityRelationGroup();
     ...
     * @param r is relation with type alternative
	 * @param from is start state for alternative
	 * @param to is end state for alternative
	 */
	protected static void alternative(final Relation r,
			                          String from,
			                          String to,
			                          final EFA efa){
		Object o = null;
		...
		while(i.hasNext() || o != null){
			...
			while(o instanceof Relation){
				...
				if(RelationType.SEQUENCE.equals(((Relation)o).getType())){
					...
					final Relation seq = factory.createRelation();

					final List<Activity> tmp = new LinkedList<Activity>();

					var_name = addWaitForNodeToFinish(tmp,(Relation)o,efa);

					seq.setType(RelationType.SEQUENCE);

					for(final Activity a : tmp){
	 ...
	 * @param efa
	 */
	protected static void uncontrollabelAlternative(final Relation r,
            										String from,
            										String to,
            										final EFA efa){
		...
		for(final Object o : objectList){
			...
			if(o instanceof Activity){
				...
				{
					final List<Activity> activityList = new LinkedList<Activity>();
					final String varName = addWaitForNodeToFinish(activityList, (Relation)o, efa);
	 ...
	 */
	private static String addWaitForNodeToFinish(final List<Activity> activityList,
												 final Relation relation, final EFA efa){
	 ...
	 */
	protected static void parallel(final Relation r, String parallel_var, final Module m){
    ...
    final String startAction = "";
		...
		for(int i = 0; i < activityRelations.length; i++){
			...
			if(activityRelations[i] instanceof Relation){
				...
				//------- set start conditions --------//
				final Activity start_par = factory.createActivity();
				...
				// Set stop conditions //
				final Activity stop_par = factory.createActivity();
	 ...
	 * @param arbitraryNode_var variable name for this arbitrary node from 0 to ant arbitrary track + 1
	 * @param m module to build arbitrary in.
	 */
	protected static void arbitrary(final Relation r, String arbitraryNode_var, final Module m){
	...
	protected static Relation addStartStopAttributes(final Relation relation,
												 	 final List<Attribute> attStartList,
												 	 final List<Attribute> attStopList){
		...
		if(RelationType.SEQUENCE.equals(relation.getType())){
			...
			//loop over all
			for(final Object o : activityRelationGroup){
				if(o instanceof Activity){
					addAttributes((Activity)o, attStartList);
					addAttributes((Activity)o, attStopList);
				}else if(o instanceof Relation){
	 ...
	 * @param attList
	 */
	private static void addAttributes(final Activity activity, final List<Attribute> attList){
