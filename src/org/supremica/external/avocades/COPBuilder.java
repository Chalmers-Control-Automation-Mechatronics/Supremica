...
public class COPBuilder {
     ...
     */
    public void setSynthesizerOptions(final SynthesizerOptions synthesizerOptions){
    ...
    )
    {
    	...
    	//Add all EOP:s
    	for( final EOP eop : adaptedEOPList ){
    		builder.add( eop );
    ...
    public List<EOP> getAdaptedEOPList(){
    	...
    	final List<EOP> tmpEOPList = new LinkedList<EOP>();
    	EOP tmpEOP;

    	for(final EOP eop: eopList){
    		for(final ROP rop : ropList){
    ...
    public List<IL> getAdaptedILList(){
    	...
    	for(final IL il : ilList){

    		/*
    		 * Create new il
    		 */
    		final IL tmpIL = Converter.copy( il );
    ...
    public List<ROP> getAdaptedROPList(){
    	...
    	for(final ROP rop : ropList){
    		final ROP tmpROP = Converter.copy( rop );
    ...
    private void addZoneBookingFromEOP(final ROP rop){
    	final List<String> preBookedZones = new LinkedList<String>();
    	addZoneBookingFromEOP(rop.getRelation(), preBookedZones);
    }

    private List<String> addZoneBookingFromEOP(final Relation relation, List<String> preBookedZones){
    	...
    	if (RelationType.SEQUENCE.equals( relation.getType() )){
    		...
    		for(final Object o : relation.getActivityRelationGroup() ){
    		...
    		for(int i = 1; i < relation.getActivityRelationGroup().size(); i++ ){
    			 ...
    			 */
    			for(final String zone : tmpPreBookedZones2){
    		...
    		if (0 == preBookedZones.size() ){
    			 ...
    			 * Remove zones that not are prebooked by all paths
    			 */
    			for(final String zone : tmpPreBookedZones1){
    ...
    private List<String> addZoneBookingFromEOP(final Activity activity,
    		                                   final List<String> preBookedZones)
    {
    	//Sanity check
    	if(null == activity){
    		return preBookedZones;
    	...
    	//Find EOP
    	final EOP eop = getFirstMatchingEOPfromList(activity, eopList);
    	...
    	for (final ZoneState zoneState : eop.getInitialState().getZoneState()){
    		...
    		if (BOOK.equals( zoneState.getState() )){
    			...
    			if ( !preBookedZones.contains( zoneState.getZone() ) ){
    				...
    				for ( final Attribute att : activity.getProperties().getAttribute() ){
    	...
    	for( final Action action : eop.getAction() ){

    		for (final ZoneState zoneState : action.getZoneState()){
    	...
    	final Action lastAction = eop.getAction().get( eop.getAction().size() - 1 );
    	for ( final ZoneState zoneState : lastAction.getZoneState() ){
    ...
    private static EOP getFirstMatchingEOPfromList(final Activity activity, final List<EOP> eopList){
    ...
    private static void addZonesFromEOPtoActivity(final EOP eop, final Activity activity){
    	...
    	for(final String zone :  eop.getZones().getZone()){
			final Attribute att = createZoneAttribute( zone );
    ...
    private static boolean contains(final List<Attribute> attributeList, final Attribute attribute){
    	...
    	for (final Attribute att : attributeList){
     ...
     * @return an attribute
     */
    private static Attribute createZoneAttribute(final String zoneName){
		...
		final UpperIndicator uppInd = factory.createUpperIndicator();
		final LowerIndicator lowInd = factory.createLowerIndicator();
    ...
    private void renameILOperationInList(final List<String> operationList){
    ...
    private void renameStringsFromMachineStrings(final List<String> operationList,
    		                               final List<String> machineList)
    {
		 ...
		 * Rename Operations in Operation list
		 */
    	for(final String op : operationList){

    		operationAdded = false;

    		for( final String opm : machineList ){
    	...
    	for(final String op : renamedOperationList){
     ...
     */
    private String removeMachineString(final String str){
    ...
    private void renameOperationsInTermList( final IL il ){
		...
		for(final Term term : il.getILStructure().getTerm() ){
			for(final OperationCheck operationCheck : term.getOperationCheck()){
    ...
    private void saveDocument( final Document document, final File file ){
    ...
    private Document openDocument( final File file ){
    	Document doc = null;
		try{
			final SAXBuilder inp = new SAXBuilder();
    ...
    private ModuleSubject getDOPtoEFAOutput(final List<ROP> adaptedROPList){
    	...
    	for(final ROP rop : adaptedROPList){
    ...
    private boolean eopIsUsedByROP(final EOP eop, final ROP rop){
    ...
    private boolean relationContainsEOP(final EOP eop, final Relation r){
    	...
    	for(final Object o : r.getActivityRelationGroup()){
    ...
    private void removeMachineNameFromOperations(final Element element){
    ...
    private void removeMachineNameFromActivitiesInROP(final Document rop){
    	...
    	final Element relation = rop.getRootElement().getChild("Relation");
    		...
    		String strMachineName )
    {
    	...
    	if( null != activity.getPrecondition() ){
    		for(final OperationReferenceType opRef : activity.getPrecondition().getPredecessor()){
    ...
    private Document watersModuleToProjectDocument(final ModuleSubject module){
    	...
    	final SupremicaMarshaller supremicaMarshaller = new SupremicaMarshaller();
        ...
        try
		{
        	final Project project = supremicaProjBuilderFromWatersModule.build( module );
    ...
    public ModuleSubject mergeModules(final List<ModuleSubject> moduleList){
    	...
    	for(final ModuleSubject mod : moduleList){

    		//Add event declarations from module
    		for(final EventDeclSubject sub : mod.getEventDeclListModifiable()){
    		...
    		for(final AbstractSubject sub : mod.getComponentListModifiable()){
    ...
    public ModuleSubject synchronizeModules(final List<ModuleSubject> moduleList){
    ...
    public ModuleSubject createSupervisor(final List<ModuleSubject> moduleList){
