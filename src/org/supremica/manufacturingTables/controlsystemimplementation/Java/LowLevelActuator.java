...
/**
 * The LowLevelActuator class describes the low level actuator
 * in order for higer level actuators to communicate with it. No alarms are given on this lower level.
 *
...
public class LowLevelActuator extends Actuator
{
    private String currentState;
    
    public LowLevelActuator(String name)
    {
	super(name);
		...
		while (sensorIter.hasNext())
			...
			if (hasState(currentSensorState))
				...
				if (!sensorExist)
					...
					//System.err.println("Broken equipment: " + name + " ! (different states in different sub level sensors)");
					return null;
				    }
			    }
		    }
	...
	if (!hardwareConnections.isEmpty())
	    {
		//start a boolean that says that the operation is started
	...
	if (!actuators.isEmpty())
		...
		// state of the actuator, which is exactly what we want to do. 
