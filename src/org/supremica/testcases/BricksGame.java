...
class BrickBuilder
{
	...
	private void addRight(int r, int c, int num, boolean reverse) throws Exception
	{
		State source = automaton.getState(shared_states[r][c]);		
		State dest = automaton.getState(shared_states[r][c+1]); // to the right
		
		Event src_dst = reverse ? new Event(dest.getId() + source.getId() + num) : new Event(source.getId() + dest.getId() + num);
		Event dst_src = reverse ? new Event(source.getId() + dest.getId() + num) : new Event(dest.getId() + source.getId() + num);
	...
	private void addDown(int r, int c, int num, boolean reverse) throws Exception
	{
		State source = automaton.getState(shared_states[r][c]);
	...
	public BrickBuilder(final int r, final int c) throws Exception
	{
		rows = r;
		cols = c;
	...
	public Automaton buildBrick(final int r, final int c) throws Exception
	{
		number = (r-1)*cols+c;
	...
	public Automaton zeroBrick() throws Exception
	{
		number = 0;
...
public class BricksGame
{
	...
	public BricksGame(int rows, int cols) throws Exception
	{
