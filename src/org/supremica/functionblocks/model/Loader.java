...
public class Loader
{
    ...
    private void loadSystem(JaxbSystem xmlSystemData)
    {
		...
		{
			JaxbDevice theDevice = (JaxbDevice) xmlSystemData.getDevice().get(0);
			if(theDevice.isSetResource())
			{
				for (Iterator resIter = theDevice.getResource().iterator();resIter.hasNext();)
				{
					JaxbResource curXmlResource = (JaxbResource) resIter.next();
					...
					if (curXmlResource.isSetFBNetwork())
					{
						...
						constructFBNetwork((JaxbFBNetwork) curXmlResource.getFBNetwork());
					}
				}
			}
		}
		...
		/*
		  ...
		  Application theApplication = (Application) xmlSystemData.getApplication().get(0);
		  ...
		  constructFBNetwork((JaxbFBNetwork) theApplication.getFBNetwork());
		  }
		  }
		*/
    ...
    private void loadFBType(JaxbFBType xmlFBTypeData)
    {
		if (xmlFBTypeData.isSetFBNetwork())
		{
			...
			constructCompositeFBType((JaxbFBNetwork) xmlFBTypeData.getFBNetwork(),newCompositeFBType);
		}
    ...
    private void constructFBNetwork(JaxbFBNetwork xmlFBNetworkData)
    {
		...
		if (xmlFBNetworkData.isSetEventConnections())
		{
			...
			{
				JaxbConnection curConn = (JaxbConnection) eventConnIter.next();
		...
		if (xmlFBNetworkData.isSetDataConnections())
		{
			...
			{
				JaxbConnection curConn = (JaxbConnection) dataConnIter.next();
    ...
    private void constructFBInterface(JaxbFBType xmlFBTypeData, FBType newFBType)
    {
		...
		if (xmlFBTypeData.isSetInterfaceList())
		{
			...
			if (xmlFBTypeData.getInterfaceList().isSetEventInputs())
			{
				...
				{
					JaxbEvent curEvent = (JaxbEvent) iter.next();
					...
					{
						With curWith = (With) withIter.next();
    ...
    private void constructBasicFBType(JaxbFBType xmlFBTypeData, BasicFBType newBasicFBType)
    {
		...
		{
			JaxbECState curState = (JaxbECState) iter.next();
			if (curStateNum == 0)
			{
			...
			{
				JaxbECAction curAction = (JaxbECAction) actionIter.next();
		...
		{
			JaxbECTransition curTrans = (JaxbECTransition) iter.next();
