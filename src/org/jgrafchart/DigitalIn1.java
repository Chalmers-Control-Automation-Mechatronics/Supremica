...
public class DigitalIn1 extends DigitalIn implements Readable {
  ...
  public void readInput() {
    ...
    if (s.compareTo("0") == 0) {
     val = false;
     if (oldval) {myBorder.setPen(redPen);myCircle.setPen(greenPen);}
    } else {
     val = true;
     if (!oldval) {myBorder.setPen(greenPen);myCircle.setPen(redPen);}
    }
    } else {
       val = !digIn.get();
//       System.out.println("Digin "+ val);
       if (!val) {myBorder.setPen(redPen);myCircle.setPen(greenPen);
                  myValue.setText("0");}
       else {myBorder.setPen(greenPen);myCircle.setPen(redPen);
             myValue.setText("1");}
    }
  }
  ...
  public void initialize() {
    ...
    if (s.compareTo("0") == 0) {
     val = true;
     myBorder.setPen(redPen);myCircle.setPen(greenPen);
    } else {
     val = false;
     myBorder.setPen(greenPen);myCircle.setPen(redPen);
    }
    oldval = val;
    } else {
       val = !digIn.get();
