...
public class MachineControlSystem extends MachineController implements Listener
{
    ...
    private final Map<String, ComponentConfirmation> componentChecks; // stores which components are checked externally
    private final Map<String, ComponentReport> componentRequests; // stores which components are requested externally

    public MachineControlSystem(final Mailbox mailbox)
    {
	super();
    ...
    public boolean performEOP(final String EOPName)
    {
	...
	// Actuators
	for (final Iterator<Entry<String,String>> actuatorIter = initialRow.getActuatorToStateMap().entrySet().iterator(); actuatorIter.hasNext();)
	{
	  final Entry<String,String> actuatorToState = actuatorIter.next();
	  sendMessage( new Message( ID, actuatorToState.getKey(), stateInquiryType, actuatorToState.getValue() ) );
	}
	// Variables
	for (final Iterator<Entry<String,String>> variableIter = initialRow.getVariableToValueMap().entrySet().iterator(); variableIter.hasNext();)
	{
	  ...
	  sendMessage( new Message( ID, variableToValue.getKey(), stateInquiryType, variableToValue.getValue() ) );
	}
	// Sensors
	for (final Iterator<Entry<String,String>> sensorIter = initialRow.getSensorToStateMap().entrySet().iterator(); sensorIter.hasNext();)
	{
	  ...
	  sendMessage( new Message( ID, sensorToState.getKey(), "monitorState", true ) );

	  sendMessage( new Message( ID, sensorToState.getKey(), stateInquiryType, (String) sensorToState.getValue() ) );
	}
	// Zones
	...
	// Actuators
	for (final Iterator<Entry<String,String>> actuatorIter = actionRow.getActuatorToStateMap().entrySet().iterator(); actuatorIter.hasNext();)
	{
	  final Entry<String,String> actuatorToState = actuatorIter.next();
	  sendMessage( new Message( ID, actuatorToState.getKey(), "orderState", actuatorToState.getValue() ) );
	...
	{
	  final Entry<String,String> variableToValue = variableIter.next();
	  sendMessage( new Message( ID, variableToValue.getKey(), "orderState", variableToValue.getValue() ) );

	}
	// Sensors
	for (final Iterator<Entry<String,String>> sensorIter = actionRow.getSensorToStateMap().entrySet().iterator(); sensorIter.hasNext();)
	{
	  final Entry<String,String> sensorToState = sensorIter.next();
	  // Changed sensor -> monitorState off
	  sendMessage( new Message( ID, sensorToState.getKey(), "monitorState", false ) );

	  sendMessage( new Message( ID, sensorToState.getKey(), "orderState", (String) sensorToState.getValue() ) );
	}
	...
	{
	  final Entry<String,String> zoneToState = zoneIter.next();
	  machine.orderZone(zoneToState.getKey(), zoneToState.getValue() );
	}
	...
	if ( msg.getType().equals("reportState") )
	{
		     ...
		     EOPIter.hasNext();)
		{
		    ...
		    {
			final String state = (String) EOPRow.getActuatorToStateMap().remove(  sender );
...
// 			    System.out.println("The entire alternative EOP was removed");
		    ...
		    {
			final String state = (String) EOPRow.getSensorToStateMap().remove(  sender );
			if (! ( state.equals( (String) msg.getContent() ) || state.equals(EOP.IGNORE_TOKEN) ) )
			{
			    EOPIter.remove();
			}
		    ...
		    {
			final String value = (String) EOPRow.getVariableToValueMap().remove(  sender );
 	...
 	if (EOPRowPerformed())
	{
		...
		// It is OK to set state monitoring off for all sensors, even those who were not started.
 		final EOPRow lastRow = currentEOP.getLastRow();
 		for (final Iterator<Entry<String,String>> sensorIter = lastRow.getSensorToStateMap().entrySet().iterator(); sensorIter.hasNext();)
 		{
 		  final Entry<String,String> sensorToState = sensorIter.next();
 		  sendMessage( new Message(ID, sensorToState.getKey(), "monitorState", false));
 		}
