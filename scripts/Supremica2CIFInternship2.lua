  ...
  for i = 1, efalist:size() do
    ...
    while edgeIterator:hasNext() do
      ...
      if gablock then
        ...
        if aexpr then aexpr = aexpr:match("[{%s,]*(.+),}") end
        
        if aexpr and aexpr ~= "" then
          local str = aexpr .. ","
          for cap in str:gmatch(patterns.commasep) do
            local lhs, op, rhs = cap:match(patterns.operatorsep)
            if lhs and op and rhs then
              ...
              local evIter = evlist:iterator()
              while evIter:hasNext() do
                local evName = sanitize(evIter:next():getName(), "event")
                
                if not GlobalActionMap[evName] then GlobalActionMap[evName] = {} end
                if not GlobalActionMap[evName][efaName] then GlobalActionMap[evName][efaName] = {} end
                if not GlobalActionMap[evName][efaName][lhs] then GlobalActionMap[evName][efaName][lhs] = {} end
                
                -- Store the location and the math!
